...
// On phones, preload the atmosphere so the first browser-permitted interaction
// can start it without waiting for the MP3 request. Respect data-saver mode.
if(mq.matches&&!navigator.connection?.saveData){
  [music,rain].forEach(track=>{if(track){track.preload='auto';track.load();}});
}

// Make the audio control recognisable at a glance on phones.
