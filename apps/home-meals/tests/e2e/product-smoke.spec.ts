...
test("preview week, physical prep and maintenance repertoire remain distinct until explicit approval",async({page})=>{
  ...
  await expect(page.getByRole("heading",{name:"Choose our prep"})).toBeVisible();
  await expect(page.getByRole("heading",{name:"At home"})).toBeVisible();
  await page.getByRole("button",{name:"Edit stock"}).click();
  await page.getByRole("button",{name:"Add one GOLD portion"}).click();
  ...
  await page.getByRole("button",{name:"Make our week",exact:true}).first().click();
  const weekDialog=page.getByRole("dialog",{name:"Our week"});
  await expect(weekDialog).toBeVisible();
  await expect(weekDialog.getByRole("button",{name:"Keep this week",exact:true})).toBeVisible();
  ...
  await page.goto("/plan",{waitUntil:"domcontentloaded"});
  const confirmDialog=page.getByRole("dialog",{name:"Our week"});
  await expect(confirmDialog).toBeVisible();
  await confirmDialog.getByRole("button",{name:"Keep this week",exact:true}).click();
