const CACHE='home-meals-v12';
const IMAGE_CACHE='home-meals-images-v3';
const CORE=['/','/cook','/cook/builder','/history','/prep','/prep/day','/prep/mids','/prep/boosters','/kitchen','/plan','/learn','/scan','/manifest.webmanifest','/icon.svg'];
...
self.addEventListener('fetch',event=>{
 ...
 if(req.destination==='image'){
  event.respondWith(caches.open(IMAGE_CACHE).then(async cache=>{const hit=await cache.match(req);if(hit)return hit;try{const res=await fetch(req);if(res.ok||res.type==='opaque'){cache.put(req,res.clone()).then(()=>trim(IMAGE_CACHE,180));}return res}catch{return hit||Response.error()}}));
  return;
 }
