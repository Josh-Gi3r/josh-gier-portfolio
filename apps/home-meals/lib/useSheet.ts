...
export function useSheet(open:boolean,onClose:()=>void){
 ...
 useEffect(()=>{if(!open)return;const old=document.body.style.overflow;const active=document.activeElement as HTMLElement|null;document.body.style.overflow="hidden";
  ...
  const focusFirst=()=>{const d=dialog();if(!d)return;const preferred=d.querySelector<HTMLElement>('[autofocus]');const first=preferred??d.querySelector<HTMLElement>(focusable);first?.focus({preventScroll:true})};
  ...
  window.addEventListener("keydown",key);return()=>{window.clearTimeout(timer);document.body.style.overflow=old;window.removeEventListener("keydown",key);active?.focus?.({preventScroll:true})}},[open]);
}
