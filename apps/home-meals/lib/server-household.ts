...
export async function writeHousehold(payload:Record<string,unknown>,baseVersion:number):Promise<StoredHousehold|"conflict">{await ensureSchema();const db=sql();if(baseVersion<=0){const rows=await db<StoredHousehold[]>`insert into home_meals_household_state (id,version,payload,updated_at) values (${HOUSEHOLD_ID},1,${db.json(payload)},now()) on conflict (id) do nothing returning id,version,payload,updated_at`;return rows[0]??"conflict"}const rows=await db<StoredHousehold[]>`update home_meals_household_state set version=${baseVersion+1},payload=${db.json(payload)},updated_at=now() where id=${HOUSEHOLD_ID} and version=${baseVersion} returning id,version,payload,updated_at`;return rows[0]??"conflict"}
