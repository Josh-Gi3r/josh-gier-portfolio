...
try{
 ...
 const dan=all.find(x=>x.id==='dan-dan-noodles');if(!dan)fail('Dan dan noodles missing');else{if(dan.prep?.length)fail('Dan dan noodles should not force DOUBAN or another prep component');if(dan.prepStrategy!=='pantry-sauce')fail('Dan dan noodles should be pantry-sauce/direct assembly');if(!dan.ingredients.some(x=>x.id==='sui-mi-ya-cai')||!dan.ingredients.some(x=>x.id==='chilli-oil'))fail('Dan dan noodles lost ya cai/chilli-oil identity');}
 const chowFun=all.find(x=>x.id==='beef-chow-fun');if(chowFun?.targetServings!==4||!chowFun.ingredients.some(x=>x.id==='fresh-wide-rice-noodles'))fail('Beef chow fun four-serving fresh-noodle formulation missing');
 const chowMein=all.find(x=>x.id==='chicken-chow-mein');if(!chowMein?.ingredients.some(x=>x.id==='hk-pan-fried-noodles'))fail('Chicken chow mein lost Hong Kong pan-fried noodle identity');
 const fried=all.find(x=>x.id==='egg-fried-rice');if(fried?.ingredients.find(x=>x.id==='egg')?.qty!==6)fail('Home egg fried rice should remain a meal-scale 6-egg four-serving formulation');
 const prawns=all.find(x=>x.id==='salt-pepper-prawns');if(prawns?.ingredients.find(x=>x.id==='large-prawns')?.qty!==600)fail('Salt & pepper prawns should remain a 600g four-serving dinner formulation');
