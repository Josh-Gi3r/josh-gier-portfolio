...
try{
 ...
 const butter=recipes.find(x=>x.id==='butter-chicken');if(!butter)fail('Butter chicken missing');else{const ids=butter.prep.map(x=>x.componentId);if(ids.join(',')!=='makhani')fail(`Butter chicken prep must be MAKHANI-only, found ${ids.join(',')}`);if(butter.ingredients.find(x=>x.ingredientId==='chicken-thigh')?.qty!==700)fail('Butter chicken should be a meal-scale 700g chicken four-serving batch');}
 ...
 const omelette=recipes.find(x=>x.id==='masala-omelette');if(omelette?.prep.length)fail('Masala omelette must remain no-base');if(omelette?.ingredients.find(x=>x.ingredientId==='egg')?.qty!==8)fail('Masala omelette should be 8 eggs for four breakfast portions');
