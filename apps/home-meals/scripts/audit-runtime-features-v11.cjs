...
must(has(scan,'capture="environment"','accept="image/*"'),"Camera/library image inputs are not both exposed");
...
must(has(cooking,'sessionStorage','wakeLock','cookMeal(id)','logMealWithoutStock(id)','rateMeal','noteMeal','saveMealPhoto','recordCookObservation'),"Cooking runtime is missing persistence, wake lock, reconciliation, ratings, notes, photos or observation logging");
...
must(has(pwa,'serviceWorker','controllerchange','/cook/'),"PWA runtime lost service-worker or active-cooking update protection");
...
console.log("\nHome Meals runtime feature audit passed · persistent Ask/camera/voice · typed AI actions require confirmation · 6 Vision scan modes · cooking resume/timers/wake-lock/reconciliation/ratings/notes/photos · 7-day Plan + shopping/prep · sync/recovery · PWA update safety · leftover/reheat policy · dock clearance");
