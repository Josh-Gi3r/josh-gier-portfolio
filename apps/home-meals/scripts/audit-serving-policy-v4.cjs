...
try{
 ...
 const recipeUi=read('components/app/Recipe.tsx'),cookUi=read('components/app/Cooking.tsx'),household=read('data/household-v12.ts');
 if(!/getRuntimeDinnerFormulationV4/.test(recipeUi)||!/getRuntimeDinnerFormulationV4/.test(cookUi))fail('recipe/cooking UI is not bound to four-serving runtime formulation');
 if(!/prepForRecipeAtCookScaleV4/.test(household))fail('cook completion does not consume four-serving prep truth');
 ...
 else console.log('\nHome Meals serving-policy v4 audit passed · 2 diners · 4-serving default cook · 3-serving option · leftovers explicit · non-linear seasoning/fat scaling · runtime/grocery/prep/cook truth aligned · GitHub Actions absent');
