...
try{
 ...
 const ingredientEngine=read('data/ingredient-engine-v2.ts'),foodEngine=read('data/food-engine-v2.ts'),runtimeDinner=read('data/runtime-dinner-v4.ts');
 if(!/getPhase2LiveRuntimeV7/.test(ingredientEngine)||!/canonicalIngredientKeyV7/.test(ingredientEngine))fail('Ingredient engine is not wired to gated V7 recipes/canonical ingredients');
 if(!/getPhase2LiveRuntimeV7/.test(foodEngine))fail('Prep engine is not wired to gated V7 recipes');
 if(!/getPhase2LiveRuntimeV7/.test(runtimeDinner))fail('Runtime dinner adapter is not wired to gated V7 recipes');
 if(fs.existsSync(path.join(repo,'.github','workflows','home-meals-ci.yml')))fail('Home Meals GitHub Actions workflow was recreated');
 if(failures.length){console.error(`\nHome Meals Live Promotion V7 readiness audit FAILED (${failures.length})`);for(const x of failures)console.error(` - ${x}`);process.exitCode=1}else console.log(`\nHome Meals Live Promotion V7 readiness audit passed · ${result.ready}/100 structurally ready · 0/100 live · ingredient catalogue ${iv.count} (${iv.phase2Added} Phase2 additions) · unit-safe canonical ingredients · 100/100 4-serving + supported 3-serving runtime adapters · deterministic ingredient/prep/runtime engines promotion-gated · 11-wave promotion registry · V6 nutrition/prep overlays locked · evidence/image briefs present · GitHub Actions absent`);
