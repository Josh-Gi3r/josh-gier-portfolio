...
export function DraftRecipe({id}:{id:string}){
 ...
 return <div className="hm-screen hm-draft-recipe">
  ...
  {standard&&<><p className="hm-lead hm-draft-subtitle">{standard.subtitle}</p><div className="hm-stats four hm-draft-stats"><div className="hm-stat"><b>{standard.minutes}</b><span>minutes</span></div><div className="hm-stat"><b>{standard.mealStyle}</b><span>meal style</span></div><div className="hm-stat"><b>{payload.servings??"—"}</b><span>servings</span></div><div className="hm-stat"><b>{standard.difficulty}</b><span>difficulty</span></div></div><div className="hm-fine hm-draft-standardline">{standard.cuisine} · {standard.methodLabel}{standard.allergens.length?` · Allergens: ${standard.allergens.join(", ")}`:" · No named allergens in the draft"}</div></>}
