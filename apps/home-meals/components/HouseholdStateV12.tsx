...
import {
 addMeasuredBatchV12,componentStockV12,cookRecipeV12,migrateHouseholdV11ToV12,setIngredientStockV12,setManualComponentStockV12,
 type HouseholdStateV12,type RatingV12,type RecipeNoteV12,type RecipeVersionV12
} from "@/data/household-v12";

const KEY="home-meals-household-v12";
const LEGACY_KEY="home-meals-household-v11";
...
function restoreV12(raw:string):HouseholdStateV12|null{try{const parsed=JSON.parse(raw);if(parsed?.version!==12||!Array.isArray(parsed.week)||!parsed.manualComponentStock||!parsed.ingredientStock)return null;return parsed as HouseholdStateV12}catch{return null}}
...
export function HouseholdStateV12Provider({children}:{children:React.ReactNode}){
 const[state,setState]=useState<HouseholdStateV12>(freshState);const[hydrated,setHydrated]=useState(false);
 useEffect(()=>{try{const exact=localStorage.getItem(KEY);const restored=exact?restoreV12(exact):null;if(restored){setState(restored)}else{const legacyRaw=localStorage.getItem(LEGACY_KEY);const legacy=legacyRaw?JSON.parse(legacyRaw):{};const migrated=migrateHouseholdV11ToV12(legacy,defaults);setState(migrated);localStorage.setItem(KEY,JSON.stringify(migrated))}}catch{setState(freshState())}finally{setHydrated(true)}},[]);
 useEffect(()=>{if(!hydrated)return;localStorage.setItem(KEY,JSON.stringify(state))},[hydrated,state]);
 ...
 const shoppingNeeds=useMemo(()=>shoppingNeedsForPlanV2(state.week.map(recipeId=>({recipeId})),state.ingredientStock),[state.week,state.ingredientStock]);
 ...
 const setIngredientObserved=(ingredientId:string,value:Quantity)=>setState(prev=>setIngredientStockV12(prev,ingredientId,value));
 ...
 const cookMeal=(recipeId:string,variantId?:string)=>{if(!validRecipeIds.has(recipeId))return;setState(prev=>cookRecipeV12(prev,recipeId,variantId))};
 ...
 const noteMeal=(recipeId:string,text:string,author:"josh"|"g"|"home"="home")=>{const clean=text.trim();if(!validRecipeIds.has(recipeId)||!clean)return;setState(prev=>{const note:RecipeNoteV12={author,text:clean.slice(0,500),at:new Date().toISOString()};return{...prev,recipeNotes:{...prev.recipeNotes,[recipeId]:[note,...(prev.recipeNotes[recipeId]??[])].slice(0,20)}}})};
 const promoteRecipeVersion=(recipeId:string,summary:string,author:"josh"|"g"|"home"="home")=>{const clean=summary.trim();if(!validRecipeIds.has(recipeId)||!clean)return;setState(prev=>{const current=prev.recipeVersions[recipeId]??[];const version:RecipeVersionV12={number:(current[0]?.number??1)+1,summary:clean.slice(0,500),author,at:new Date().toISOString()};return{...prev,recipeVersions:{...prev.recipeVersions,[recipeId]:[version,...current].slice(0,20)}}})};
 const toggleUseSoon=(ingredientId:string)=>setState(prev=>{const next=!prev.useSoon[ingredientId];const useSoon={...prev.useSoon,[ingredientId]:next},useSoonAt={...prev.useSoonAt};if(next)useSoonAt[ingredientId]=new Date().toISOString();else delete useSoonAt[ingredientId];return{...prev,useSoon,useSoonAt}});
 const toggleFavourite=(recipeId:string)=>{if(!validRecipeIds.has(recipeId))return;setState(prev=>({...prev,favourites:{...prev.favourites,[recipeId]:!prev.favourites[recipeId]}}))};
 ...
 const value={state,componentStock,prepNeeds,shoppingNeeds,setDay,setComponentObserved,setIngredientObserved,recordMeasuredBatch,cookMeal,rateMeal,noteMeal,promoteRecipeVersion,toggleUseSoon,toggleFavourite,toggleGrocery,clearMigrationWarnings,resetV12};
