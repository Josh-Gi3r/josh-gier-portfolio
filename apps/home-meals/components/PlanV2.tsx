...
export function PlanV2(){
 ...
 const favourites=meals.filter(m=>{const r=h.ratings[m.id];return (r?.josh??0)>=4||(r?.g??0)>=4});
 const cookedIds=new Set(h.history.map(x=>x.mealId));
 const notRecent=meals.filter(m=>!cookedIds.has(m.id)&&!favourites.includes(m)).slice(0,5);
 const newTests=meals.filter(m=>m.status==="placeholder").slice(0,4);
 const emergency=meals.filter(m=>m.minutes<=25).slice(0,4);
 const weekend=meals.filter(m=>m.minutes>=30).slice(0,4);
 ...
 const pool=[{label:"Favourites",items:(favourites.length?favourites:meals.slice(0,4))},{label:"Not had recently",items:notRecent},{label:"New to test",items:newTests},{label:"Fast backups",items:emergency},{label:"Weekend",items:weekend}];
 return <div className="hm-screen hm-plan-screen hm-v3-screen">
  ...
  <section className="hm-section hm-v3-section hm-month-pool"><div className="hm-v3-section-head"><div><span>THIS MONTH</span><h2>Our meal pool</h2></div><small>repeat the good ones · test a few</small></div><div className="hm-month-pool-grid">{pool.map(group=><article key={group.label}><header><strong>{group.label}</strong><span>{group.items.length}</span></header>{group.items.length?group.items.slice(0,4).map(m=><Link href={`/cook/${m.id}`} key={m.id}>{m.image&&<img src={m.image} alt=""/>}<span><strong>{m.title}</strong><small>{m.minutes}m · {m.cuisine}</small></span></Link>):<p>Nothing here yet. It will fill as we cook and rate.</p>}</article>)}</div></section>
