...
export function Shell({children}:{children:React.ReactNode}){
 ...
 const answer=(text:string)=>{const s=text.toLowerCase();
  if(!h.kitchenReady&&(s.includes("have")||s.includes("buy")||s.includes("prep")||s.includes("freezer")))return "I don't know the kitchen yet. Check Fridge, Freezer and Pantry once, then I can use the real stock.";
  ...
  return "Ask about tonight, groceries, prep, or what we have at home.";
 };
 ...
 return <div className="hm-shell-v5">
  ...
  {ask&&<div className="hm-sheet-backdrop-v5" role="presentation" onMouseDown={e=>{if(e.target===e.currentTarget)setAsk(false)}}><section className="hm-sheet-v5 hm-ask-sheet-v5" role="dialog" aria-modal="true" aria-label="Ask Home">
   ...
   <div className="hm-ask-context-v5"><div><span>Tonight</span><strong>{tonight.title}</strong></div><Link href={`/cook/${tonight.id}`} onClick={()=>setAsk(false)}>Open recipe</Link></div>
   <div className="hm-ask-messages-v5">{messages.slice(-6).map((m,i)=><div key={i} className={m.who}>{m.text}</div>)}</div>
   <div className="hm-ask-quick-v5"><button onClick={()=>send("What should we cook tonight?")}>Tonight</button><button onClick={()=>send("What do we need to buy?")}>Groceries</button><button onClick={()=>send("What should we prep?")}>Prep</button><Link href="/scan" onClick={()=>setAsk(false)}>Camera</Link></div>
