...
export function Shell({children}:{children:React.ReactNode}){
 ...
 return <div className="hm-shell-v5 hm-shell-v6">
  ...
  {ask&&<div className="hm-sheet-backdrop-v5" role="presentation" onMouseDown={e=>{if(e.target===e.currentTarget)setAsk(false)}}><section className="hm-sheet-v5 hm-ask-sheet-v5 hm-ask-sheet-v6 hm-ask-sheet-v10" role="dialog" aria-modal="true" aria-label="Ask Home">
   ...
   <div className="hm-ask-messages-v5 hm-ask-messages-v10" tabIndex={0} aria-label="Ask Home conversation">{messages.slice(-6).map((m,i)=><div key={i} className={m.who}><p>{m.text}</p>{m.tags&&<div className="hm-ask-tags-v10">{m.tags.map(tag=><span key={tag}>{tag}</span>)}</div>}{m.mealIds&&<div className="hm-ask-meals-v10">{m.mealIds.map(id=>{const r=getRecipe(id);const title=recipeTitle(r.id,r.title);return <Link key={id} href={`/cook/${id}`} onClick={()=>setAsk(false)}>{r.image?<img src={r.image} alt={title}/>:<i/>}<span><strong>{title}</strong><small>{r.minutes} min · {r.cuisine}</small></span><b>›</b></Link>})}</div>}{m.href&&m.action&&<Link className="hm-ask-action-v10" href={m.href} onClick={()=>setAsk(false)}>{m.action} <b>→</b></Link>}</div>)}</div>
