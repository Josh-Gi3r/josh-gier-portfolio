...
export function Shell({children}:{children:React.ReactNode}){
 ...
 const answer=(text:string):HomeReply=>{const s=text.toLowerCase();
  ...
  if(s.includes("prep")||s.includes("freezer")||s.includes("low")){const a=h.prepNeeds.slice(0,6).map(x=>{const c=prepComponents.find(y=>y.id===x.id);return c?`${c.code} · ${x.shortMl} ml`:""}).filter(Boolean);return a.length?{text:`${h.prepNeeds.length} ${h.prepNeeds.length===1?"prep job is":"prep jobs are"} short for this week.`,tags:a,href:"/prep/day",action:"Start Prep Day"}:{text:"The freezer covers this week.",href:"/prep",action:"Open Prep"}}
  ...
  const ingredient=ingredients.find(x=>s.includes(x.name.toLowerCase())||s.includes(x.id.replace(/-/g," ")));if(ingredient){const n=h.ingredientStock[ingredient.id]??0;const state=ingredient.tracking==="state"?(n>0?"we have it":"we're out"):n>0?formatQty(n,ingredient.unit):"out";return {text:`${ingredient.name}: ${state}.`,tags:h.useSoon[ingredient.id]?["Use soon"]:undefined,href:"/kitchen",action:"Open Kitchen"}}
  if(s.includes("have")||s.includes("at home")||s.includes("kitchen")){const stockedIngredients=ingredients.filter(x=>(h.ingredientStock[x.id]??0)>0).length;const stockedMothers=prepComponents.filter(x=>x.kind==="mother"&&(h.componentStock[x.id]??0)>0).length;const soon=Object.keys(h.useSoon).filter(id=>h.useSoon[id]&&(h.ingredientStock[id]??0)>0).length;return {text:`At home: ${stockedIngredients} tracked ingredients and ${stockedMothers}/8 mother bases stocked.`,tags:soon?[`${soon} use soon`]:["Nothing urgent"],href:"/kitchen",action:"Open Kitchen"};}
  return {text:"Ask me about tonight, what we have, what can be made now, groceries, prep, use-soon food, favourites, or the last dinner."};
 }
