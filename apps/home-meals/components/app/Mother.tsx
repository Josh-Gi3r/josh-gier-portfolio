...
export function Mother({id}:{id:string}){const h=useHousehold();const m=motherBases.find(x=>x.id===id)!;const recipe=baseRecipesV2[id];const mids=midBases.filter(x=>x.parentMotherIds.includes(id));const linked=recipes.filter(r=>r.motherIds.includes(id));const[confirm,setConfirm]=useState(false);const ml=h.componentStock[id]??0;const portions=stockPortions(id,h.componentStock);const batchMl=batchOutputMl(id);
