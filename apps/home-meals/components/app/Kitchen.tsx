...
const packetNoun=(kind:PrepPacketKindV6,n=1)=>{const word=kind==="stock-block"?"stock block":kind==="booster-dose"?"dose":kind==="pantry-dose"?"pantry dose":kind==="fridge-portion"?"fridge portion":"meal packet";return`${word}${n===1?"":"s"}`};
...
export function Kitchen(){
 ...
 const lowMother=motherBases.find(m=>h.prepNeeds.some(n=>n.id===m.id&&n.shortQty>0));
 ...
 const says=(()=>{
  ...
  if(tab==="Freezer"){
   if(lowMother)return{text:<><b>{lowMother.code}</b> is short for this week. Prep Day will ask for the actual finished weight or volume, then Home will show full storage packets plus any loose remainder.</>,actions:<Link className="primary" href="/prep/day">Prep it</Link>};
   ...
   return{text:<>The recorded freezer covers the week. Home keeps exact g/ml truth and uses recorded batches oldest first.</>};
  }
