...
export function Kitchen(){
 ...
 const oldestC=oldest?getComponent(oldest.componentId):null;
 ...
 const hero={
  ...
  Freezer:{img:foundationImages.freezer,heading:h.kitchenReady?`${stockedMothers} bases · ${pucks} working portions`:"Count the bases",sub:h.kitchenReady?(oldest&&oldestC?`oldest measured batch: ${oldestC.code} from ${new Date(oldest.producedAt).toLocaleDateString(undefined,{day:"numeric",month:"short"})}`:lowMother?`${lowMother.code} is low for this week`:"measured batches used oldest first"):"grams or ml, per base"},
 ...
 const says=(()=>{
  if(!h.kitchenReady)return{text:<>Teach me the kitchen once. Pantry staples can stay qualitative; exact amounts are only used when we genuinely measured them.</>,actions:<><button className="primary" onClick={()=>{h.confirmKitchen();feedback("success")}}>Kitchen checked</button><Link className="ghost" href={scanHref}>Use the camera</Link></>};
  if(tab==="Fridge"){
   ...
   return{text:<>Tap anything to update the amount you actually have. Home won’t invent expiry or quantity.</>};
  }
  if(tab==="Freezer"){
   if(lowMother)return{text:<><b>{lowMother.code}</b> is low for this week. Prep Day will ask for the real finished weight or volume before adding stock.</>,actions:<Link className="primary" href="/prep/day">Prep it</Link>};
   if(oldest&&oldestC)return{text:<><b>{oldestC.code}</b> from {new Date(oldest.producedAt).toLocaleDateString(undefined,{day:"numeric",month:"short"})} is the oldest measured batch — use it first.</>};
   return{text:<>The recorded freezer covers the week. Measured batches are consumed FIFO.</>};
  }
 ...
 return <div className="hm-screen">
  ...
  <div className="hm-seg" role="tablist">{tabs.map(t=><button key={t} role="tab" aria-selected={tab===t} className={tab===t?"on":""} onClick={()=>switchTab(t)}>{t}</button>)}</div>
  ...
  {tab==="Freezer"?<>
   <div className="hm-card lg hm-wheel-card"><div className="hm-wheel" style={{background:wheel(h.componentStock)}}><div className="inner"><div><b>{pucks}</b><span>working portions</span>{lowMother&&<small>{lowMother.code} low</small>}</div></div></div><div className="hm-wheel-legend">{motherBases.map(m=><span key={m.id} style={{"--tone":toneFor(m.id)} as CSSProperties}><i/>{m.code} <small>{stockPortions(m.id,h.componentStock)}</small></span>)}</div></div>
   <div className="hm-list">{motherBases.map(m=>{const n=stockPortions(m.id,h.componentStock),truth=getCanonicalPrepV2(m.id),heroImg=motherHero(m.id),old=activeBatches.find(b=>b.componentId===m.id),planned=h.week.some(id=>recipePrepV2(id).some(p=>p.componentId===m.id));if(!truth)return null;return <div key={m.id} className="hm-card hm-freezer-row" style={{"--tone":toneFor(m.id),"--tone-grad":toneGradient(m.id)} as CSSProperties}><button className="thumb" onClick={()=>setEditComponent(m.id)} style={{overflow:"hidden",padding:0}}>{heroImg?<img src={heroImg} alt="" style={{width:"100%",height:"100%",objectFit:"cover"}}/>:m.code}</button><button style={{textAlign:"left",minWidth:0}} onClick={()=>setEditComponent(m.id)}><strong>{m.code} <small>· {m.name}</small></strong><div className="hm-segs">{[0,1,2,3,4,5,6,7].map(i=><i key={i} className={i<n?"on":""}/>)}</div><span className="sub">{h.componentStock[m.id]??0} {truth.workingUnit.unit} · {n<=0?"Out":n<2&&planned?"Low · Prep Sunday":old?`oldest ${new Date(old.producedAt).toLocaleDateString(undefined,{day:"numeric",month:"short")}`:planned?"used this week":"good"}</span></button><div className="hm-stepper"><button className="minus" onClick={()=>h.setComponent(m.id,Math.max(0,(h.componentStock[m.id]??0)-truth.workingUnit.qty))}>−</button><b>{n}</b><button className="plus" onClick={()=>h.setComponent(m.id,(h.componentStock[m.id]??0)+truth.workingUnit.qty)}>+</button></div></div>})}</div>
   <MidsAndBoosters onEdit={setEditComponent}/>
  </>:<>
   <label className="hm-search"><input value={q} onChange={e=>setQ(e.target.value)} placeholder={`Find in the ${tab.toLowerCase()}`}/>{q&&<button type="button" className="clear" onClick={()=>setQ("")}>×</button>}</label>
...
function MidsAndBoosters({onEdit}:{onEdit:(id:string)=>void}){const h=useHousehold(),[more,setMore]=useState(false),activeIds=new Set(h.week.flatMap(id=>recipePrepV2(id).map(x=>x.componentId))),other=prepComponents.filter(c=>c.kind!=="mother").sort((a,b)=>Number(activeIds.has(b.id))-Number(activeIds.has(a.id))||Number((h.componentStock[b.id]??0)>0)-Number((h.componentStock[a.id]??0)>0)||a.code.localeCompare(b.code)),shown=more?other:other.filter(x=>activeIds.has(x.id)||(h.componentStock[x.id]??0)>0).slice(0,8);return <><SectionHead title="Mids & boosters" action={<Link href="/prep/mids">Explore ›</Link>}/>{shown.length?<div className="hm-list tight">{shown.map(c=>{const n=stockPortions(c.id,h.componentStock),truth=getCanonicalPrepV2(c.id);if(!truth)return null;return <div key={c.id} className="hm-card hm-row" style={{padding:"8px 14px 8px 10px"}}><button className="hm-dot" style={{"--tone":toneFor(c.id),width:44,height:44,borderRadius:14,background:toneGradient(c.id),color:"#fff",fontSize:10,fontWeight:800,display:"grid",placeItems:"center"} as CSSProperties} onClick={()=>onEdit(c.id)}>{c.code.slice(0,5)}</button><button style={{textAlign:"left",minWidth:0}} onClick={()=>onEdit(c.id)}><strong>{c.code}</strong><small>{c.name}{activeIds.has(c.id)?" · this week":""}</small></button><div className="hm-stepper"><button className="minus" onClick={()=>h.setComponent(c.id,Math.max(0,(h.componentStock[c.id]??0)-truth.workingUnit.qty))}>−</button><b>{n}</b><button className="plus" onClick={()=>h.setComponent(c.id,(h.componentStock[c.id]??0)+truth.workingUnit.qty)}>+</button></div></div>})}</div>:<div className="hm-empty"><strong>No mids or boosters counted yet.</strong>Add them after making and measuring a batch.</div>}<button className="hm-cook-more" onClick={()=>setMore(v=>!v)}>{more?"Show less":`Show all ${other.length}`}</button></>}
