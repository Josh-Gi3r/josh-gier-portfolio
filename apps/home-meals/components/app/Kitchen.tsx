...
export function Kitchen(){
 ...
 return <div className="hm-page-v5 hm-kitchen-v5">
  <PageHead eyebrow="KITCHEN" title="Kitchen" sub="What we have at home." action={<Link className="hm-round-link-v5" href={scanHref} aria-label={`Open ${scanMode.toLowerCase()} camera`}>⌁</Link>}/>
  ...
  {tab==="Freezer"?<Freezer/>:<>
   <label className="hm-search-v5"><span>⌕</span><input value={q} onChange={e=>setQ(e.target.value)} placeholder={`Find in ${tab.toLowerCase()}`} aria-label={`Find in ${tab.toLowerCase()}`}/></label>
  ...
  <div className="hm-kitchen-bottom-v5"><Link href={scanHref}>Use camera</Link><button onClick={()=>{h.confirmKitchen();feedback("success")}}>Kitchen checked ✓</button></div>
 </div>
}
...
function Freezer(){
 ...
 return <>
  ...
  <section className="hm-block-v5">
   ...
   <div className="hm-freezer-other-v5">{shown.map(c=>{const n=stockPortions(c.id,h.componentStock);return <article key={c.id} style={{"--tone":c.tone} as React.CSSProperties}><i/><span><strong>{c.code}</strong><small>{c.name}{activeIds.has(c.id)?" · this week":""}</small></span><div className="hm-stepper-v5"><button aria-label={`Decrease ${c.code}`} onClick={()=>setByPortion(c.id,-1)}>−</button><b>{n}</b><button aria-label={`Increase ${c.code}`} onClick={()=>setByPortion(c.id,1)}>+</button></div></article>})}</div>
   <button className="hm-text-button-v5" onClick={()=>{setMore(v=>!v);feedback("tap")}}>{more?"Show less":"Show all prep"}</button>
  </section>
 </>;
}
