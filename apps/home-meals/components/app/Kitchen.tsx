...
export function Kitchen(){
 ...
 return <div className="hm-screen">
  ...
  {tab==="Freezer"?<FreezerPanel componentStock={h.componentStock} week={h.week} prepBatches={h.prepBatches} setComponent={h.setComponent} onEdit={setEditComponent}/>:<>
   <label className="hm-search"><input value={q} onChange={e=>setQ(e.target.value)} placeholder={`Find in the ${tab.toLowerCase()}`} aria-label={`Find in the ${tab.toLowerCase()}`}/>{q&&<button type="button" className="clear" onClick={()=>setQ("")}>×</button>}</label>
