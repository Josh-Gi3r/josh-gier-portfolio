...
function FreezerPanel({componentStock,week,prepBatches,setComponent,onEdit}:{componentStock:Record<string,number>;week:string[];prepBatches:{componentId:string;remaining:{qty:number};producedAt:string}[];setComponent:(id:string,qty:number)=>void;onEdit:(id:string)=>void}){
 ...
 return <>
  ...
  <div className="hm-list">{motherBases.map(m=>{const n=stockPortions(m.id,componentStock),truth=getCanonicalPrepV2(m.id);if(!truth)return null;const heroImg=motherHero(m.id),old=activeBatches.find(b=>b.componentId===m.id),planned=week.some(id=>recipePrepV2(id).some(p=>p.componentId===m.id));let status="good";if(n<=0)status="Out";else if(n<2&&planned)status="Low · Prep Sunday";else if(old)status=`oldest ${new Date(old.producedAt).toLocaleDateString(undefined,{day:"numeric",month:"short"})}`;else if(planned)status="used this week";return <div key={m.id} className="hm-card hm-freezer-row" style={{"--tone":toneFor(m.id),"--tone-grad":toneGradient(m.id)} as CSSProperties}><button className="thumb" onClick={()=>onEdit(m.id)} style={{overflow:"hidden",padding:0}}>{heroImg?<img src={heroImg} alt="" style={{width:"100%",height:"100%",objectFit:"cover"}}/>:m.code}</button><button style={{textAlign:"left",minWidth:0}} onClick={()=>onEdit(m.id)}><strong>{m.code} <small>· {m.name}</small></strong><div className="hm-segs">{[0,1,2,3,4,5,6,7].map(i=><i key={i} className={i<n?"on":""}/>)}</div><span className="sub">{componentStock[m.id]??0} {truth.workingUnit.unit} · {status}</span></button><div className="hm-stepper"><button className="minus" onClick={()=>setComponent(m.id,Math.max(0,(componentStock[m.id]??0)-truth.workingUnit.qty))}>−</button><b>{n}</b><button className="plus" onClick={()=>setComponent(m.id,(componentStock[m.id]??0)+truth.workingUnit.qty)}>+</button></div></div>})}</div>
