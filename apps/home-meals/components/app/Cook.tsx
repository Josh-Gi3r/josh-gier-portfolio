...
const PAGE=12;
type Mode="For us"|"All"|"Cuisine"|"By prep"|"Recent"|"Not lately"|"Never cooked"|"Favourites"|"Quick"|"Ready now";
const modes:Mode[]=["For us","Cuisine","By prep","Recent","Not lately","Never cooked","Favourites","Quick","Ready now","All"];
...
export function Cook(){
 ...
 const usedPrep=useMemo(()=>canonicalPrepComponentsV2.filter(c=>recipes.some(r=>recipePrepV2(r.id).some(x=>x.componentId===c.id))).sort((a,b)=>Number(h.activePrepIds.includes(b.id))-Number(h.activePrepIds.includes(a.id))||a.tier.localeCompare(b.tier)||a.code.localeCompare(b.code)),[h.activePrepIds]);
 const score=(r:typeof recipes[number])=>{
  const rd=ready(r),rating=Math.max(h.ratings[r.id]?.josh??0,h.ratings[r.id]?.g??0),fav=!!h.favourites[r.id],activeFit=h.activePrepIds.length?recipeSupportedByActivePrepV2(r.id,h.activePrepIds):false;
 ...
 const list=useMemo(()=>recipes.filter(r=>{
  if(q&&!(`${recipeTitle(r.id,r.title)} ${recipeSubtitle(r.id,r.subtitle)} ${r.title} ${r.cuisine} ${r.tags.join(" ")}`).toLowerCase().includes(q.toLowerCase()))return false;
  const hx=history.recipes[r.id];
  if(mode==="Cuisine")return !sub||r.cuisine===sub;
  if(mode==="By prep")return !sub||recipePrepV2(r.id).some(x=>x.componentId===sub);
  if(mode==="Recent")return hx.daysSince!=null&&hx.daysSince<14;
  if(mode==="Not lately")return hx.daysSince!=null&&hx.daysSince>=28;
  if(mode==="Never cooked")return hx.neverCooked;
  if(mode==="Favourites")return !!h.favourites[r.id]||(h.ratings[r.id]?.josh??0)>=4||(h.ratings[r.id]?.g??0)>=4;
  if(mode==="Quick")return r.minutes<=25;
  if(mode==="Ready now")return ready(r).state==="ready";
  return true;
 }).sort((a,b)=>mode==="Recent"?(history.recipes[a.id].daysSince??9999)-(history.recipes[b.id].daysSince??9999):mode==="Not lately"?(history.recipes[b.id].daysSince??0)-(history.recipes[a.id].daysSince??0):mode==="For us"?score(a)-score(b):Number(!!h.favourites[b.id])-Number(!!h.favourites[a.id])||recipeTitle(a.id,a.title).localeCompare(recipeTitle(b.id,b.title))),[q,mode,sub,history,h.favourites,h.ratings,h.activePrepIds,h.componentStock,h.ingredientStock,h.kitchenReady]);
 ...
 const secondary=mode==="Cuisine"?cuisines.map(x=>({id:x,label:x})):mode==="By prep"?usedPrep.map(x=>({id:x.id,label:`${x.code}${h.activePrepIds.includes(x.id)?" · active":""}`})):[];
 return <div className="hm-screen">
  ...
  <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginTop:14}}><span className="hm-note">{list.length} {list.length===1?"dish":"dishes"}</span>{mode==="For us"&&<span className="hm-note">ratings · prep · recency</span>}</div>
  {shown.length?<div className="hm-cook-grid">{shown.map((r,i)=>{const title=recipeTitle(r.id,r.title),rd=ready(r),fav=!!h.favourites[r.id],hx=history.recipes[r.id],prepFit=h.activePrepIds.length&&recipeSupportedByActivePrepV2(r.id,h.activePrepIds);let meta=r.cuisine;if(mode==="Recent"&&hx.daysSince!=null)meta=hx.daysSince===0?"Today":hx.daysSince===1?"Yesterday":`${hx.daysSince} days ago`;else if(mode==="Not lately"&&hx.daysSince!=null)meta=`${hx.daysSince} days since last`;else if(mode==="Never cooked")meta="Never cooked";else if(prepFit)meta=`${r.cuisine} · active prep`;return <Link key={r.id} href={`/cook/${r.id}`} className={`hm-cook-card ${i%3===0?"tall":""}`} style={{animationDelay:`${Math.min(i,8)*35}ms`}} onClick={()=>feedback("tap")}><div className="photo">{r.image&&<img src={r.image} alt={title} loading={i<4?"eager":"lazy"} decoding="async"/>}<span className={`fav ${fav?"on":""}`} aria-hidden="true">♥</span><span className="min">{r.minutes} min</span></div><div className="body"><strong>{title}</strong><small>{meta}</small><span className={`hm-pill ${rd.pillClass}`}><i/>{rd.label}</span></div></Link>})}</div>:<div className="hm-state" style={{minHeight:"auto",padding:"40px 0 20px"}}><div className="center"><Orb size={96}/><h1>Nothing in that view yet.</h1><p>Try another cuisine, prep item or history view, or ask Home.</p><button className="hm-btn primary sm" onClick={openAsk}>Ask Home</button></div></div>}
