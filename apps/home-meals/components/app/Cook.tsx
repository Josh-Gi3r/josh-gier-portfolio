...
export function Cook(){
 ...
 const list=useMemo(()=>recipes.filter(r=>{
  ...
  if(q&&!(`${recipeTitle(r.id,r.title)} ${recipeSubtitle(r.id,r.subtitle)} ${r.title} ${r.cuisine} ${r.tags.join(" ")} ${energy?.mealWeight??""}`).toLowerCase().includes(q.toLowerCase()))return false;
  const hx=history.recipes[r.id];
  if(mode==="Cuisine")return !sub||r.cuisine===sub;
  if(mode==="Meal weight")return !sub||energy?.mealWeight===sub;
 ...
 const secondary=mode==="Cuisine"?cuisines.map(x=>({id:x,label:x})):mode==="Meal weight"?weights:mode==="By prep"?usedPrep.map(x=>({id:x.id,label:`${x.code}${h.activePrepIds.includes(x.id)?" · active":""}`})):[];
