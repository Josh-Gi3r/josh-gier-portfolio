...
export function Mids(){
 ...
 return <div className="hm-page-v5 hm-mids-v5"><Back href="/prep" label="Prep"/><PageHead eyebrow="MID-BASES" title="Mid-bases" sub="Small batches that turn the same mother base into different dinners."/>
  ...
  {filtered.length?<div className="hm-mid-list-v5">{filtered.map(m=>{const n=counts[m.id]??0;const thisWeek=weekIds.has(m.id);return <Link href={`/prep/mids/${m.id}`} key={m.id} style={{"--tone":m.tone} as React.CSSProperties}><i/><div><strong>{m.code}</strong><span>{m.name}</span><small>{m.parentMotherIds.length?m.parentMotherIds.map(id=>motherBases.find(x=>x.id===id)?.code).filter(Boolean).join(" + "):"stands alone"}</small></div><div className={`hm-mid-maturity-v5 ${n?"active":"idle"}`}>{thisWeek?"this week":n?`${n} saved ${n===1?"recipe":"recipes"}`:"not in our rotation yet"}</div><b>›</b></Link>})}</div>:<div className="hm-empty-v5"><strong>{scope==="week"&&!activeFilters?"No mid-base needed this week.":"Nothing matches."}</strong><p>{scope==="week"&&!activeFilters?"The current dinners either use mother bases directly or standalone prep.":"Clear a filter or try another search."}</p>{activeFilters>0&&<button className="hm-text-button-v5" onClick={()=>{setCuisine("All");setParent("all");feedback("tap")}}>Clear filters</button>}</div>}
