...
export function Mids(){
 ...
 const mother=motherBases.find(x=>x.id===selected)??motherBases[0],weekIds=useMemo(()=>new Set(h.week.flatMap(id=>prepForRecipeAtCookScaleV7(id).map(x=>x.componentId))),[h.week]),related=useMemo(()=>mids.filter(x=>x.madeFrom.includes(mother.id)||x.usedWith.includes(mother.id)),[mother.id]);
 const dinners=useMemo(()=>{const ids=new Set(related.map(x=>x.id));return recipes.filter(r=>prepForRecipeAtCookScaleV7(r.id).some(x=>ids.has(x.componentId))||prepForRecipeAtCookScaleV7(r.id).some(x=>x.componentId===mother.id))},[mother.id,related]);
 ...
 return <div className="hm-screen">
  ...
  <SectionHead title="All mids" action={<span className="muted">{mids.length} in the library</span>}/><label className="hm-search"><input value={q} onChange={e=>setQ(e.target.value)} placeholder="Find a mid or sauce" aria-label="Find a mid or sauce"/>{q&&<button type="button" className="clear" aria-label="Clear" onClick={()=>setQ("")}>×</button>}</label><div className="hm-list tight">{shownAll.map(m=>{const n=dinnersFor(m.id).length;return <Link key={m.id} href={`/prep/mids/${m.id}`} className="hm-card hm-row hm-lift" style={{padding:"10px 14px 10px 10px"}}><MidThumb id={m.id} code={m.code}/><span><strong>{m.name}</strong><small>{prepRelationshipLabelV7(m.id)}</small></span><span className={`hm-pill ${weekIds.has(m.id)?"":n?"sky":"neutral"}`}>{weekIds.has(m.id)?"this week":n?`${n} ${n===1?"dinner":"dinners"}`:"idea"}</span></Link>})}</div>{!q&&all.length>8&&<button className="hm-cook-more" onClick={()=>{setShowAll(v=>!v);feedback("tap")}}>{showAll?"Show fewer":`Show all ${all.length}`}</button>}{shownAll.length===0&&<div className="hm-empty"><strong>Nothing matches.</strong>Try another name.</div>}
 </div>;
}
