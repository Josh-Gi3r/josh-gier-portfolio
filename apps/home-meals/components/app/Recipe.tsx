...
export function Recipe({id}:{id:string}){
 const h=useHousehold();const r=getLiveRecipeV7(id);const f=getRuntimeDinnerFormulationV7(id);if(!r||!f)throw new Error(`Missing live dinner formulation: ${id}`);const ready=useReadiness()(r);const kcal=kcalReferenceForV3(id);
 ...
 const events=useMemo(()=>[...cooked.map(x=>({at:x.at,title:"Cooked",text:"Dinner logged",dot:"#6fd39a"})),...(h.recipeNotes[id]??[]).map(x=>({at:x.at,title:`${who(x.author)}’s note`,text:x.text,dot:"#2fae6e"})),...versions.map(v=>({at:v.at,title:`v${v.number} · ${v.summary}`,text:`Saved by ${who(v.author)}`,dot:"#2fae6e"}))].sort((a,b)=>new Date(b.at).getTime()-new Date(a.at).getTime()).slice(0,6),[cooked,h.recipeNotes,id,versions]);
 ...
 return <div className="hm-screen flush">
  ...
  <div className="hm-sheetpage" style={{paddingBottom:140}}>
   ...
   <SectionHead title="How it goes" action={<span className="muted">{f.steps.length} steps</span>}/><div className="hm-overview">{phases.map((p,i)=>counts[i]?<i key={p.name} style={{flex:counts[i],background:p.bar}}/>:null)}</div><div className="hm-overview-labels">{phases.map((p,i)=>counts[i]?<span key={p.name}>{p.name}</span>:null)}</div>
   ...
   <div className="hm-card hm-ref"><p>{r.balance}</p><a href={r.source.url} target="_blank" rel="noreferrer">{r.source.label} ↗</a></div>
  </div>
