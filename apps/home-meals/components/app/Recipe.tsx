...
export function Recipe({id}:{id:string}){
 ...
 const cooked=h.history.filter(x=>x.mealId===id);const lastNote=h.recipeNotes[id]?.[0];const rating=h.ratings[id]??{};
 const saveNote=()=>{h.noteMeal(id,note,"home");setSaved(true);feedback("success");setTimeout(()=>setSaved(false),1400)};
 return <div className="hm-page-v5 hm-recipe-v5"><Back href="/cook" label="Recipes"/>
  ...
  {(cooked.length>0||lastNote)&&<section className="hm-our-note-v5"><span>OUR VERSION</span><div className="hm-couple-rating-v5"><b>J</b><strong>{rating.josh?`${rating.josh}★`:"—"}</strong><b>G</b><strong>{rating.g?`${rating.g}★`:"—"}</strong></div>{lastNote&&<p>“{lastNote.text}”</p>}<small>{cooked.length?`Cooked ${cooked.length} ${cooked.length===1?"time":"times"}`:"Not cooked yet"}</small></section>}
  ...
  {r.prep.length>0&&<section className="hm-recipe-section-v5"><h2>From prep</h2><div className="hm-prep-list-v5">{r.prep.map(x=>{const c=getComponent(x.id);const have=h.componentStock[x.id]??0;return <div key={x.id}><ComponentPill id={x.id} amountMl={x.totalMl}/><span className={h.kitchenReady&&have>=x.totalMl?"ok":"need"}>{h.kitchenReady?have>=x.totalMl?"have it":`${Math.max(0,x.totalMl-have)} ml short`:"not checked"}</span></div>})}</div></section>}
