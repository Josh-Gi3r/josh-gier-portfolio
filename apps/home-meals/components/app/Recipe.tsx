...
export function Recipe({id}:{id:string}){
 const h=useHousehold();const r=getRecipe(id);const[chooseDay,setChooseDay]=useState(false);const[note,setNote]=useState(h.ratings[id]?.note??"");const[saved,setSaved]=useState(false);useSheet(chooseDay,()=>setChooseDay(false));
 ...
 return <div className="hm-page-v5 hm-recipe-v5">
  ...
  <section className="hm-recipe-hero-v5">{r.image?<img src={r.image} alt=""/>:<div className="hm-photo-placeholder-v5">{title[0]}</div>}<div className="hm-recipe-hero-shade-v5"/><div className="hm-recipe-hero-copy-v5"><div><span>{r.cuisine}</span><RecipeReady recipe={r}/></div><h1>{title}</h1><p>{subtitle}</p><small>{r.minutes} min · {r.method} · serves 2</small></div></section>
  ...
  <section className="hm-recipe-section-v5"><h2>What we need</h2><div className="hm-ingredient-list-v5">{r.ingredients.map(x=>{const d=getIngredient(x.id);const onHand=h.ingredientStock[x.id]??0;const missing=h.kitchenReady&&ingredientRequirementMissing(x,h.ingredientStock);const status=!h.kitchenReady?"not checked":d?.tracking==="state"?(onHand>0?"have":"out"):missing?(onHand>0?`${formatQty(onHand,x.unit)} at home`:"out"):"have enough";return <div key={`${x.id}-${x.raw}`}><span>{d?.name??x.id}</span><div style={{textAlign:"right"}}><strong style={{display:"block"}}>{formatQty(x.qty,x.unit)}</strong><small style={{display:"block",fontSize:11,marginTop:2,color:h.kitchenReady?(missing?"#9b4737":"#2f684d"):"var(--muted)",fontWeight:h.kitchenReady?750:400}}>{status}</small></div></div>})}</div></section>
