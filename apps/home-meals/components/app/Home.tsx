...
export function Home(){
 const h=useHousehold(),ready=useReadiness(),day=(new Date().getDay()+6)%7,[idx,setIdx]=useState(0);
 ...
 const displayWeek=h.weekStatus==="unplanned"?[]:h.weekStatus==="suggested"&&h.suggestedWeek?.length===7?h.suggestedWeek:h.week,confirmed=h.weekStatus==="confirmed";
 const stack=useMemo(()=>{
  const chosenId=displayWeek[day]??displayWeek[0],chosen=chosenId?getLiveRecipeV7(chosenId):null;
  ...
  const readyNow=h.kitchenReady&&h.weekStatus!=="unplanned"?ranked.filter(r=>recipeAvailabilityV7(r.id,h.componentStock,h.ingredientStock).ready):ranked;
  const candidates=readyNow.length?readyNow:ranked;
  const first=chosen??candidates[0]??recipes[0],rest=candidates.filter(r=>r.id!==first.id);return[first,...rest]
 ...
 const says=(()=>{
  if(!h.kitchenReady)return{text:<>I don’t know what’s in the kitchen yet. Add what we have, or show me with the camera, and I can tell us what we can make.</>,actions:<Link className="primary" href="/kitchen">Check the kitchen</Link>};
  if(!confirmed&&h.weekStatus==="suggested")return{text:<>I made us a week. Have a look and change anything you don’t fancy.</>,actions:<Link className="primary" href="/plan">Review our week</Link>};
  if(!confirmed&&!h.activePrepIds.length)return{text:<>Kitchen’s set. Want me to make us a week, or pick a few things we like keeping prepped first?</>,actions:<><Link className="primary" href="/prep">Pick our prep</Link><Link className="ghost" href="/plan">Make a week</Link></>};
  if(!confirmed)return{text:<>Want me to make us a week around what we have and the prep we like keeping around?</>,actions:<Link className="primary" href="/plan">Make our week</Link>};
  if(idx>0){const ago=tonightHistory.daysSince;if(tonightReady.state==="ready")return{text:<>Okay, not that one. <b>{tonightTitle}</b> is ready with what we have{ago!=null?`, and we last had it ${ago} ${ago===1?"day":"days"} ago`:""}.</>};return{text:<>Okay, not that one. Here’s another idea: <b>{tonightTitle}</b>{ago!=null?`. We last had it ${ago} ${ago===1?"day":"days"} ago.`:"."}</>};}
  if(tonightReady.state==="ready")return{text:<>Tonight looks like <b>{tonightTitle}</b> — {tonight.minutes} minutes, we have what we need{tonightActiveFit?", and it fits your usual prep":""}{tonightHistory.daysSince!=null?`. Last cooked ${tonightHistory.daysSince} ${tonightHistory.daysSince===1?"day":"days"} ago.`:". It hasn’t been logged before."}</>};
  ...
  return{text:<>Tonight is <b>{tonightTitle}</b>, but we’re short {missing||"a few things"}. Swap it, or try another idea.</>,actions:<><Link className="primary" href="/plan">Swap it</Link><button className="ghost" onClick={notTonight}>Another idea</button></>};
 })();
