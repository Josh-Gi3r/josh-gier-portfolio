...
export function Home(){
 ...
 const tonight=stack[idx%stack.length],tonightTitle=recipeTitle(tonight.id,tonight.title),tonightReady=ready(tonight),tonightPrep=recipePrepV2(tonight.id);
 ...
 const pucks=motherBases.reduce((n,m)=>n+stockPortions(m.id,h.componentStock),0),stockedMothers=motherBases.filter(m=>(h.componentStock[m.id]??0)>0).length;
 ...
 const last=h.history[0]?getRecipe(h.history[0].mealId):null,lastPhoto=last?h.mealPhotos.find(x=>x.mealId===last.id):undefined;
 const says=(()=>{
  ...
  if(idx>0)return{text:<>Okay, not that one. Here’s the next from what we have — {tonight.minutes} minutes{tonightReady.state==="ready"?", nothing recorded as missing.":"."}</>};
  if(tonightReady.state==="ready")return{text:<>Tonight looks like <b>{tonightTitle}</b> — {tonight.minutes} reference minutes, and recorded Kitchen truth covers it. Not feeling it? Say so and I’ll pull the next idea.</>};
 ...
 if(!welcomeSeen&&!h.kitchenReady&&h.history.length===0)return <div className="hm-screen hm-home-empty"><header className="hm-home-head"><h1 className="hm-h1">Hey Josh &amp; G</h1></header><div className="stage"><Orb size={140} label="Home"/></div><div className="copy"><h2>Let’s start with what’s in the kitchen.</h2><p className="hm-lead">Show me the fridge and freezer once, and everything else — the week, shopping, prep — gets real.</p></div><div className="acts"><Link className="hm-btn primary" href="/scan?mode=Fridge&back=%2Fkitchen" onClick={dismissWelcome}>Show Home the fridge</Link><Link className="hm-btn ghost" href="/plan" onClick={dismissWelcome}>Pick this week’s dinners first</Link></div><button className="foot" onClick={()=>{dismissWelcome();window.dispatchEvent(new Event("home-meals:ask"))}}>Or just say “what’s for dinner?”</button></div>;
 ...
 return <div className="hm-screen">
  ...
  <div className="hm-pulse" aria-label="Kitchen pulse"><Link href="/kitchen" style={{"--accent":"var(--peach-text)","--tint-grad":"linear-gradient(180deg,#fff,#fff1ea)"} as CSSProperties}><span>Fridge</span><b>{h.kitchenReady?soonIds.length:"—"}</b><small>{h.kitchenReady?(soonIds.length?"use soon":"nothing urgent"):"not checked"}</small></Link><Link href="/kitchen"><span>Freezer</span><b>{h.kitchenReady?pucks:"—"}</b><small>{h.kitchenReady?(lowMother?`portions · ${lowMother.code} low`:`portions · ${stockedMothers}/${motherBases.length} bases`):"not checked"}</small></Link><Link href="/plan"><span>To buy</span><b>{h.kitchenReady?h.shoppingNeeds.length:"—"}</b><small>{h.kitchenReady?"for the week":"check kitchen"}</small></Link></div>
  {last&&<Link href={`/cook/${last.id}`} className="hm-card hm-memory hm-lift">{(lastPhoto?.dataUrl||last.image)&&<img src={lastPhoto?.dataUrl??last.image} alt={recipeTitle(last.id,last.title)} loading="lazy"/>}<div><span className="kick">LAST COOKED{lastPhoto?" · OUR PHOTO":""}</span><strong>{recipeTitle(last.id,last.title)}</strong><small>{h.ratings[last.id]?.josh?`Josh ${h.ratings[last.id].josh}★`:"Josh —"} · {h.ratings[last.id]?.g?`G ${h.ratings[last.id].g}★`:"G —"}{h.recipeNotes[last.id]?.[0]?` · “${h.recipeNotes[last.id][0].text}”`:""}</small></div><span className="arrow">›</span></Link>}
 </div>;
}
