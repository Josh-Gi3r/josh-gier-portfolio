...
export function Prep(){
 ...
 return <div className="hm-page-v5 hm-prep-v5">
  ...
  <section className="hm-prep-week-v5"><div><span>THIS WEEK</span><h2>{h.kitchenReady?(h.prepNeeds.length?`${h.prepNeeds.length} ${h.prepNeeds.length===1?"thing needs":"things need"} topping up`:"We’re covered"):`${demand.length} prep ${demand.length===1?"item":"items"} in the plan`}</h2><p>{h.kitchenReady?(h.prepNeeds.length?"Only make what the week is short of.":"Nothing needs making for the current plan."):"Check the freezer to turn this into a real shortfall list."}</p></div><div>{h.kitchenReady?<Link href={h.prepNeeds.length?"/prep/day":"/kitchen"}>{h.prepNeeds.length?"Start prep":"Freezer"}</Link>:<Link href="/kitchen">Check freezer</Link>}</div></section>
