...
export function Prep(){
 ...
 return <div className="hm-page-v5 hm-prep-v5">
  ...
  <BaseExplorer/>
  {h.prepBatches.length>0&&<section className="hm-block-v5"><SectionHead eyebrow="RECENT" title="What we made"/><div className="hm-batch-history-v5">{h.prepBatches.slice(0,5).map((b,i)=>{const c=getComponent(b.componentId);return <div key={`${b.at}-${i}`}><i style={{background:c?.tone}}/><span><strong>{c?.code}</strong><small>{new Date(b.at).toLocaleDateString(undefined,{day:"numeric",month:"short"})}</small></span><b>{b.outputMl} ml</b></div>})}</div></section>}
