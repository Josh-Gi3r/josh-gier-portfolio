...
export function MealHistory(){
 ...
 return <div className="hm-screen">
  ...
  <p className="hm-lead hm-gut" style={{marginTop:10,fontSize:14}}>This is what we’ve cooked, when we had it, and what each of us thought. I use it to keep dinner ideas from getting repetitive.</p>
  ...
  {summary.cuisines30.length>0&&<><SectionHead title="Last 30 days" action={<span className="muted">what we’ve been eating</span>}/><div className="hm-chiplist">{summary.cuisines30.map(x=><span key={x.cuisine}>{x.cuisine} · {x.count}</span>)}</div></>}
