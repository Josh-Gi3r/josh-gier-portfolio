...
export function Cooking({id}:{id:string}){
 ...
 if(done)return <div className="hm-cooking-v5 hm-cooking-complete-v5"><main><section className="hm-cook-finished-v5"><span>✓</span><small>DINNER LOGGED</small><h1>{title}</h1><h2>How was it?</h2><div className="hm-cook-finish-ratings-v5">{(["josh","g"] as const).map(who=><div key={who}><strong>{who==="josh"?"Josh":"G"}</strong><div>{[1,2,3,4,5].map(n=><button key={n} className={(h.ratings[id]?.[who]??0)>=n?"on":""} aria-label={`${who} ${n} stars`} onClick={()=>{h.rateMeal(id,who,n);feedback("change")}}>★</button>)}</div></div>)}</div><label className="hm-cook-note-v5"><span>Next time</span><textarea value={note} onChange={e=>setNote(e.target.value)} placeholder="More chilli? Less sweet? Different side?"/><button disabled={!note.trim()} onClick={saveNote}>{saved?"Saved ✓":"Save note"}</button></label><div className="hm-cook-complete-actions-v5"><Link href={`/cook/${id}`}>Recipe</Link><Link href="/">Home</Link></div></section></main></div>;
