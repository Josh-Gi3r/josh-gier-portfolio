...
export function Cooking({id}:{id:string}){
 const h=useHousehold();const r=getRecipe(id);const[step,setStep]=useState(0);const[done,setDone]=useState(false);const[timer,setTimer]=useState(0);const[note,setNote]=useState("");const[saved,setSaved]=useState(false);const tick=useRef<ReturnType<typeof setInterval>|null>(null);const title=recipeTitle(r.id,r.title);
 ...
 const finish=()=>{if(done)return;h.cookMeal(id);if(tick.current){clearInterval(tick.current);tick.current=null}setTimer(0);setDone(true);feedback("success")};
 ...
 return <div className="hm-cooking-v5 hm-cooking-v6">
