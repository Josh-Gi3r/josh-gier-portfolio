...
export function Cooking({id}:{id:string}){
 const h=useHousehold();const r=getRecipe(id);const[step,setStep]=useState(0);const[done,setDone]=useState(false);const[timer,setTimer]=useState(0);const tick=useRef<ReturnType<typeof setInterval>|null>(null);
 ...
 return <div className="hm-cooking-v5">
  ...
  <button className="hm-cook-ask-v5" onClick={()=>document.querySelector<HTMLButtonElement>(".hm-ask-fab-v5")?.click()}>✦ Ask Home</button>
 </div>
}
