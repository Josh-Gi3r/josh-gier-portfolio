...
export function Scan(){const[mode,setMode]=useState<Mode>("Fridge");const[preview,setPreview]=useState<string>();const[back,setBack]=useState("/");
 ...
 const clearPreview=()=>{if(preview)URL.revokeObjectURL(preview);setPreview(undefined)};
 ...
 return <div className="hm-page-v5 hm-scan-v5"><Back href={back} label={backLabel}/><PageHead eyebrow="CAMERA" title="Show Home" sub="Take a photo. We’ll still update the kitchen ourselves for now."/>
  ...
  <section className="hm-camera-v5">{preview?<img src={preview} alt={`Selected ${mode.toLowerCase()} photo`}/>:<div><span>{modes.find(x=>x.id===mode)?.icon}</span><strong>{mode==="Prep"?"Check the prep":mode}</strong><p>{mode==="Prep"?"Photograph the pan or prep stage.":mode==="Receipt"?"Photograph the whole receipt clearly.":mode==="Meal"?"Photograph the finished dinner.":`Photograph the ${mode.toLowerCase()}.`}</p></div>}<label><input type="file" accept="image/*" capture="environment" aria-label={`Choose ${mode.toLowerCase()} photo`} onChange={e=>{const f=e.target.files?.[0];if(f){clearPreview();setPreview(URL.createObjectURL(f));feedback("change")}}}/><span>{preview?"Take another":"Camera / photo"}</span></label></section>
