...
function Photo({c}:{c:CanonicalPrepComponentV2}){const image=prepHero(c.id);return image?<img src={image} alt={c.name} loading="lazy"/>:<span style={{background:toneGradient(c.id)}}/>}
...
const meta:Record<Family,{eyebrow:string;title:string;desc:string;image:string}>={
 bases:{eyebrow:"FOUNDATIONS",title:"Core bases",desc:"Seven foundations. Pick the one you want, then make it or see what it unlocks.",image:"/images/prep-v9/core-bases.webp"},
 mids:{eyebrow:"BUILDERS",title:"Mid bases & pastes",desc:"Pastes, stocks, roux and cooked flavour builders. Two big cards per row, no list hunting.",image:prepHero("rendang")??"/images/prep-v9/prep-library.webp"},
 sauces:{eyebrow:"SAUCES",title:"Sauces & condiments",desc:"Stir-fry sauces, marinades, tare and condiments, organised as tappable food cards.",image:prepHero("wok-brown")??"/images/prep-v9/prep-library.webp"},
 boosters:{eyebrow:"BOOSTERS",title:"Boosters",desc:"Small concentrated preps with outsized flavour impact.",image:prepHero("ginger-garlic")??"/images/prep-v9/prep-library.webp"},
 common:{eyebrow:"SHORTCUTS",title:"Common prep",desc:"Everyday shortcuts from across the system. This is a convenience view, not a new food tier.",image:"/images/prep-v9/prep-library.webp"},
};
...
function PrepFamilyV10({family}:{family:Family}){
 ...
 return <div className="hm-screen hm-prep-family-v10">
  ...
  <div className="hm-prep-family-v10-hero"><img src={info.image} alt=""/><div className="shade"/><div className="copy"><span>{info.eyebrow}</span><h2>{items.length} {info.title}</h2><p>{info.desc}</p></div></div>
