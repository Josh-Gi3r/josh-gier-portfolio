...
export function PrepV10(){
 ...
 const packetText=(id:string)=>{const p=getPrepPortionPolicyV6(id),qty=h.componentStock[id]??0;if(!p)return `${qty} recorded`;const n=formatPacketCountV6(qty/p.packet.qty),noun=p.kind==="stock-block"?"block":p.kind==="booster-dose"?"dose":p.kind==="fridge-portion"?"portion":p.kind==="pantry-dose"?"dose":"packet";return `${n} ${noun}${n==="1"?"":"s"} · ${formatQty(qty,p.packet.unit)}`};
 ...
 return <div className="hm-screen hm-prep-v10">
  ...
  {mode==="week"&&<section className="hm-prep-v10-view" aria-label="This week's prep">
   {h.weekStatus!=="confirmed"?<div className="hm-prep-v10-week-message hm-gut"><span>PREVIEW ONLY</span><h2>This week is not locked yet.</h2><p>Review or approve the week before Home turns possible gaps into jobs.</p><Link className="hm-btn primary" href="/plan">Review Plan</Link></div>:<div className="hm-prep-v10-week-message hm-gut ready"><span>CONFIRMED WEEK</span><h2>{weekNeed.length?`${weekNeed.length} prep ${weekNeed.length===1?"job":"jobs"} to make.`:"Prep is covered."}</h2><p>{weekNeed.length?"Only the real shortages are jobs. Covered prep stays visible below.":"Nothing needs making unless you want to stock up."}</p><Link className="hm-btn primary" href="/prep/day">Open Prep Day</Link></div>}
   {weekNeed.length>0&&<><SectionHead title={h.weekStatus==="confirmed"?"Need to make":"Preview shortages"} action={<span className="muted">one job per card</span>}/><div className="hm-prep-v10-tasks">{weekNeed.map(c=>{const need=needsById.get(c.id)!,p=getPrepPortionPolicyV6(c.id);return <Link key={c.id} href={hrefFor(c)} className="hm-prep-v10-task"><div className="photo"><PrepPhoto c={c}/><div className="shade"/><span>{c.code}</span></div><div className="copy"><small>{c.tier==="mother"?"Core base":c.tier==="mid"?"Mid / sauce":"Booster"}</small><h3>{c.name}</h3><strong>{h.weekStatus==="confirmed"?"Need":"Preview short"} {formatQty(need.shortQty,need.unit)}</strong>{p&&<p>{formatQty(p.packet.qty,p.packet.unit)} per storage unit</p>}<b>{h.weekStatus==="confirmed"?`Make ${c.code}`:"Open prep"} →</b></div></Link>})}</div></>}
   {weekCovered.length>0&&<><SectionHead title="Already covered" action={<span className="muted">no job needed</span>}/><div className="hm-prep-v10-grid compact">{weekCovered.map(c=><PrepStateCard key={c.id} c={c} stock={h.componentStock[c.id]??0} packetText={packetText(c.id)} secondary="Covered for this week"/>)}</div></>}
   {!weekComponents.length&&<div className="hm-empty"><strong>No prep dependencies in this week.</strong>The selected dinners are direct-ingredient dishes.</div>}
  </section>}
 </div>
}
...
function PrepStateCard({c,stock,packetText,secondary,action}:{c:CanonicalPrepComponentV2;stock:number;packetText:string;secondary:string;action?:ReactNode}){
 return <article className="hm-prep-v10-state-card"><Link href={hrefFor(c)} className="photo"><PrepPhoto c={c}/><div className="shade"/><span className="code">{c.code}</span>{stock>0&&<span className="stock">stocked</span>}</Link><div className="body"><strong>{c.name}</strong><small>{stock>0?packetText:"None recorded"}</small><span>{secondary}</span>{action}</div></article>
}
