...
export function Boosters(){
 ...
 return <div className="hm-screen">
  ...
  <p className="hm-lead hm-gut" style={{marginTop:10,fontSize:14}}>Small aromatics, marinades and finishes. Keep only the ones your dinners actually use.</p>
...
export function BoosterDetail({slug}:{slug:string}){
 ...
 const n=portionsOf(slug,h.componentStock),tone=toneFor(slug),grad=toneGradient(slug),active=h.activePrepIds.includes(slug),inputs=f.ingredientInputs.map(x=>`${x.name} ${formatQty(x.qty,x.unit)}${x.optional?" (optional)":""}`),cues=f.method.map(x=>x.cue).filter((x):x is string=>!!x);
 ...
 return <div className="hm-screen flush" style={{"--tone":tone,"--tone-grad":grad} as CSSProperties}>
  ...
  <div className="hm-sheetpage" style={{paddingBottom:140}}>
   <h1 className="hm-h1" style={{fontSize:28,lineHeight:1.1}}>{b.name}</h1><p className="hm-lead" style={{marginTop:8}}>{about.summary}</p>
   <div className="hm-stats"><Stat v={h.kitchenReady?n:"—"} k="working portions left"/><Stat v={formatQty(b.workingUnit.qty,b.workingUnit.unit)} k="working amount" tint="var(--tint-peach)"/><Stat v={dinners.length} k={dinners.length===1?"dinner":"dinners"} tint="var(--tint-sky)"/></div>
   ...
   <SectionHead title="Canonical formulation" action={about.storage&&<span className="muted">{about.storage}</span>}/><div className="hm-chiplist">{inputs.map(x=><span key={x}>{x}</span>)}</div>
   <div className="hm-steps" style={{marginTop:20}}>{f.method.map((s,i)=><div key={s.instruction} className="hm-card hm-step" style={{"--phase":grad} as CSSProperties}><b>{i+1}</b><div><p>{s.instruction}</p>{s.cue&&<small>{s.cue}</small>}</div><span/></div>)}</div>
   {cues.length>0&&<div className="hm-cues" style={{marginTop:12}}>{cues.map(c=><div key={c} className="hm-card"><b>✓</b>{c}</div>)}</div>}
   <SectionHead title="Goes into" action={<span style={{color:tone,fontWeight:700,fontSize:13}}>{dinners.length?`${dinners.length} dinners`:"library"}</span>}/>{dinners.length?<div className="hm-rail">{dinners.map(r=><MealTile key={r.id} recipe={r}/>)}</div>:<div className="hm-empty"><strong>No saved dinner uses this yet.</strong>Keep it dormant until one does.</div>}
   {(about.note||f.evidence.length>0)&&<div className="hm-card hm-refs">{about.note&&<p>{about.note}</p>}{f.evidence.map(e=><a key={e.url} href={e.url} target="_blank" rel="noreferrer">{e.label} ↗</a>)}</div>}
  </div>
