...
export function MealCard({recipe,compact=false,day}:{recipe:CanonicalRecipe;compact?:boolean;day?:string}){const title=recipeTitle(recipe.id,recipe.title);return <Link href={`/cook/${recipe.id}`} className={`hm-meal-card-v5 ${compact?"compact":""}`} onClick={()=>feedback("tap")}>{recipe.image?<img src={recipe.image} alt="" width={320} height={240} loading="lazy" decoding="async"/>:<div className="hm-photo-placeholder-v5">{title.slice(0,1)}</div>}<div className="hm-meal-card-copy-v5">{day&&<span>{day}</span>}<strong>{title}</strong><small>{recipe.minutes} min · {recipe.cuisine}</small><RatingLine recipeId={recipe.id}/></div></Link>}
