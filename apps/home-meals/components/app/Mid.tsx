...
export function Mid({id}:{id:string}){
 ...
 const[confirm,setConfirm]=useState(false),[count,setCount]=useState(false),[toast,setToast]=useState("");const[made,setMade]=useState("");const[error,setError]=useState("");
 ...
 const finishBatch=()=>{const measured=Number(made);if(!(measured>0))return;try{const output=quantity(measured,policy.packet.unit),split=packetBreakdownV6(id,output);h.recordMeasuredProduction(id,output,"portion-v6");setMade("");setError("");setConfirm(false);feedback("success");const rem=split.remainder.qty>0?` + ${formatQty(split.remainder.qty,split.remainder.unit)} left over`:"";setToast(`${code} · ${split.fullPackets} full ${split.fullPackets===1?"portion":"portions"}${rem}`);window.setTimeout(()=>setToast(""),2200)}catch(e){setError(e instanceof Error?e.message:"Couldn’t save that batch.")}};
 const parentLine=parents.map(p=>`${p.code} ${formatQty(f.componentInputs.find(x=>x.componentId===p.id)?.qty??0,f.componentInputs.find(x=>x.componentId===p.id)?.unit??p.workingUnit.unit)}`).join(" + "),what=editorial?.what??truth.note??"A make-ahead flavour prep.";
 const says=(()=>{if(researchOnly)return{text:<>Nothing we cook right now uses <b>{code}</b>. Leave it for later.</>};if(!h.kitchenReady)return{text:<>Count the freezer first. Then I can tell you whether <b>{code}</b> is actually needed.</>,actions:<button className="primary" onClick={()=>setCount(true)}>Count {code}</button>};if(usedThisWeek&&packets===0)return{text:<><b>{code}</b> is needed this week and there isn’t any here. Make a batch, measure what you ended up with, then portion it for storage.</>,actions:<button className="primary" onClick={()=>setConfirm(true)}>Make {code}</button>};if(batches[0])return{text:<><b>{code}</b> has about {packets} full {packets===1?"portion":"portions"}. Oldest is from {new Date(batches[0].producedAt).toLocaleDateString(undefined,{day:"numeric",month:"short"})}; use it first.</>};return{text:<>{linked.length} of our dinners use <b>{code}</b>. {h.kitchenReady?`${packets} full ${packets===1?"portion":"portions"} left.`:""}</>}})();
 return <div className="hm-screen flush" style={{"--tone":tone,"--tone-grad":grad} as CSSProperties}>
  ...
  <div className="hm-sheetpage" style={{paddingBottom:140}}>
   ...
   <div className="hm-stats"><Stat v={h.kitchenReady?packets:"—"} k="portions left"/><Stat v={formatQty(policy.packet.qty,policy.packet.unit)} k="each portion" tint="var(--tint-peach)"/><Stat v={linked.length} k={linked.length===1?"dinner":"dinners"} tint="var(--tint-sky)"/></div>
   ...
   <SectionHead title="What goes in" action={<span className="muted">make a batch · measure when done</span>}/>
   ...
   {batches.length>0&&<><SectionHead title="In the freezer" action={<span className="muted">oldest first</span>}/><div className="hm-card hm-batches-card">{batches.slice(0,4).map((b,i)=>{const left=packetBreakdownV6(id,b.remaining),madeN=packetBreakdownV6(id,b.initial);return <div className="row" key={b.batchId}><b className={`n ${i===0?"first":""}`}>{i+1}</b><span><strong>{left.fullPackets} full {left.fullPackets===1?"portion":"portions"}{left.remainder.qty?` + ${formatQty(left.remainder.qty,left.remainder.unit)} left over`:""}{i===0?" · use first":""}</strong><small>Made {new Date(b.producedAt).toLocaleDateString(undefined,{day:"numeric",month:"short"})} · {formatQty(b.initial.qty,b.initial.unit)} total · {madeN.fullPackets} full portions</small></span></div>})}</div></>}
   <div className="hm-card hm-refs"><p><b>Storage · </b>{containerGuidanceV6(id)}</p>{storage?.note&&<p><b>Keep it · </b>{storage.note}</p>}{f.evidence.map(e=><a key={e.url} href={e.url} target="_blank" rel="noreferrer">{e.label} ↗</a>)}</div>
  ...
  <Sheet open={confirm} onClose={()=>{setConfirm(false);setError("")}} label={`Make ${code}`} title={`Make ${code}`} action={<span className="muted">measure when done</span>}>
   <HomeSays className="tight">Follow the recipe{parentLine?<>. It uses <b>{parentLine}</b> from what’s already prepped</>:null}. After cooling, weigh or measure what you actually made. I’ll turn that into full portions plus any amount left over.</HomeSays>
   <div className="hm-card" style={{marginTop:14,padding:14}}><strong>One portion · {formatQty(policy.packet.qty,policy.packet.unit)}</strong><small style={{display:"block"}}>{containerGuidanceV6(id)} Usually keep {policy.targetRotation[0]}–{policy.targetRotation[1]} portions around; this week suggests about {suggested}.</small></div>
   <label style={{display:"grid",gap:8,marginTop:18}}><span className="hm-note">How much you made ({policy.packet.unit})</span><input inputMode="decimal" value={made} onChange={e=>setMade(e.target.value)} placeholder={`e.g. ${Math.round(policy.packet.qty*policy.targetRotation[0])}`} aria-label={`Measured finished ${policy.packet.unit} of ${code}`}/></label>
   {error&&<p role="alert" style={{color:"var(--peach-text)",fontWeight:700,marginTop:10}}>{error}</p>}
   <div className="hm-sheet-actions"><button className="hm-btn ghost" onClick={()=>setConfirm(false)}>Not yet</button><button className="hm-btn primary" disabled={!(Number(made)>0)} onClick={finishBatch}>Add to Kitchen</button></div>
  </Sheet>
