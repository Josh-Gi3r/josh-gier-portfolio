...
export function Mid({id}:{id:string}){
 ...
 const packets=stockPortions(id,h.componentStock),tone=toneFor(id),grad=toneGradient(id),batches=useMemo(()=>h.prepBatches.filter(b=>b.componentId===id&&b.remaining.qty>0).sort((a,b)=>new Date(a.producedAt).getTime()-new Date(b.producedAt).getTime()),[h.prepBatches,id]);
