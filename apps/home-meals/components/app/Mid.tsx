...
export function Mid({id}:{id:string}){
 const h=useHousehold();const m=midBases.find(x=>x.id===id)!;const c=midContent[id];const linked=recipes.filter(r=>r.midIds.includes(id));const usedThisWeek=h.week.some(rid=>getRecipe(rid).midIds.includes(id));const researchOnly=linked.length===0;const[confirm,setConfirm]=useState(false);const[added,setAdded]=useState(false);useSheet(confirm,()=>setConfirm(false));const ml=h.componentStock[id]??0;const portions=stockPortions(id,h.componentStock);const batchMl=batchOutputMl(id);
