...
export function Mid({id}:{id:string}){const h=useHousehold();const m=midBases.find(x=>x.id===id)!;const c=midContent[id];const linked=recipes.filter(r=>r.midIds.includes(id));const[confirm,setConfirm]=useState(false);const ml=h.componentStock[id]??0;const portions=stockPortions(id,h.componentStock);const batchMl=batchOutputMl(id);
 return <div className="hm-page-v5 hm-mid-v5"><Back href="/prep/mids" label="Mids"/><header className="hm-mid-head-v5" style={{"--tone":m.tone} as React.CSSProperties}><i/><div><span>MID-BASE</span><h1>{m.name}</h1><p>{c?.what??"A make-ahead flavour direction."}</p><div>{m.parentMotherIds.length?m.parentMotherIds.map(pid=><Link href={`/prep/${pid}`} key={pid}>{motherBases.find(x=>x.id===pid)?.code}</Link>):<b>Standalone</b>}</div></div></header>
  ...
  <section className="hm-block-v5"><SectionHead eyebrow="OUR RECIPES" title={linked.length?`${linked.length} ${linked.length===1?"recipe":"recipes"} using ${m.code}`:"Not in our rotation yet"}/>{linked.length?<div className="hm-meal-rail-v5">{linked.map(r=><MealCard recipe={r} key={r.id} compact/>)}</div>:<div className="hm-empty-v5"><strong>Research is ready. Household recipes come next.</strong><p>{c?.dinners.slice(0,4).join(" · ")}</p></div>}</section>
