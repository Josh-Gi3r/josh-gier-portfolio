...
export function Mid({id}:{id:string}){
 ...
 const linked=recipes.filter(r=>recipePrepV2(r.id).some(p=>p.componentId===id)),usedThisWeek=h.week.some(rid=>recipePrepV2(rid).some(p=>p.componentId===id)),researchOnly=linked.length===0;
 ...
 const portions=stockPortions(id,h.componentStock),tone=toneFor(id),grad=toneGradient(id),batches=useMemo(()=>h.prepBatches.filter(b=>b.componentId===id&&b.remaining.qty>0).sort((a,b)=>new Date(a.producedAt).getTime()-new Date(b.producedAt).getTime()),[h.prepBatches,id]);
 const needed=h.prepNeeds.find(x=>x.id===id),suggested=needed?Math.max(1,Math.ceil(needed.shortQty/truth.workingUnit.qty)):Math.max(2,linked.length?Math.min(4,linked.length):2);
 const finishBatch=()=>{const n=Math.round(Number(made));if(!(n>0))return;try{h.recordPortionedProduction(id,n);setMade("");setError("");setConfirm(false);feedback("success");setToast(`${code} · ${n} working ${n===1?"portion":"portions"} added`);window.setTimeout(()=>setToast(""),1800)}catch(e){setError(e instanceof Error?e.message:"Couldn’t log those portions.")}};
 ...
 const says=(()=>{if(researchOnly)return{text:<>Nothing in the current 36 dinners consumes <b>{code}</b> yet. Keep it dormant until a dinner really needs it.</>};if(!h.kitchenReady)return{text:<>Count the freezer first. Then Home can tell you whether <b>{code}</b> is actually needed.</>,actions:<button className="primary" onClick={()=>setCount(true)}>Count {code}</button>};if(usedThisWeek&&portions===0)return{text:<><b>{code}</b> is required this week and none is recorded. Make it, portion it for future dinners, and tell Home how many usable portions went away.</>,actions:<button className="primary" onClick={()=>setConfirm(true)}>Make {code}</button>};if(batches[0])return{text:<><b>{code}</b> has {portions} working {portions===1?"portion":"portions"}. Oldest batch is from {new Date(batches[0].producedAt).toLocaleDateString(undefined,{day:"numeric",month:"short"})}; use it first.</>};return{text:<>{linked.length} of our dinners use <b>{code}</b>. {h.kitchenReady?`${portions} working ${portions===1?"portion":"portions"} recorded.`:""}</>}})();
 return <div className="hm-screen flush" style={{"--tone":tone,"--tone-grad":grad} as CSSProperties}>
  ...
  <div className="hm-sheetpage" style={{paddingBottom:140}}>
   ...
   <div className="hm-stats"><Stat v={h.kitchenReady?portions:"—"} k="working portions left"/><Stat v={formatQty(truth.workingUnit.qty,truth.workingUnit.unit)} k="per working portion" tint="var(--tint-peach)"/><Stat v={linked.length} k={linked.length===1?"dinner":"dinners"} tint="var(--tint-sky)"/></div>
   ...
   <SectionHead title="Canonical formulation" action={<span className="muted">portion after cooking</span>}/>
  ...
  <Sheet open={confirm} onClose={()=>{setConfirm(false);setError("")}} label={`Make ${code}`} title={`Make ${code}`} action={<span className="muted">no batch weighing</span>}>
   <HomeSays className="tight">Follow the formulation{parentLine?<>. It uses <b>{parentLine}</b> from recorded prep</>:null}. Then divide the finished prep into working portions. Home needs the count, not the total batch weight.</HomeSays>
   <div className="hm-card" style={{marginTop:14,padding:14}}><strong>One working portion = {formatQty(truth.workingUnit.qty,truth.workingUnit.unit)}</strong><small style={{display:"block"}}>A sensible target here is {suggested} {suggested===1?"portion":"portions"}; make more or less if useful.</small></div>
   <label style={{display:"grid",gap:8,marginTop:18}}><span className="hm-note">How many working portions did you put away?</span><input inputMode="numeric" value={made} onChange={e=>setMade(e.target.value)} placeholder={String(suggested)} aria-label={`Working portions of ${code} made`}/></label>
   {error&&<p role="alert" style={{color:"var(--peach-text)",fontWeight:700,marginTop:10}}>{error}</p>}
   <div className="hm-sheet-actions"><button className="hm-btn ghost" onClick={()=>setConfirm(false)}>Not yet</button><button className="hm-btn primary" disabled={!(Number(made)>0)} onClick={finishBatch}>Add portions to Kitchen</button></div>
  </Sheet>
