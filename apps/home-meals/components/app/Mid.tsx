...
export function Mid({id}:{id:string}){
 const h=useHousehold();const m=midBases.find(x=>x.id===id)!;const c=midContent[id];const f=getPrepFormulationV2(id);
 ...
 // Editorial content first; the canonical v2 formulation is the fallback for anything without a write-up yet.
 const batch=c?.batch??f?.ingredientInputs.map(x=>`${x.name} ${x.qty} ${x.unit}`)??[];const method=c?.method??f?.method.map(x=>x.instruction)??[];const what=c?.what??"A make-ahead flavour direction.";
 ...
 return <div className="hm-screen flush" style={{"--tone":tone,"--tone-grad":grad} as CSSProperties}>
  ...
  <div className="hm-sheetpage" style={{paddingBottom:140}}>
   ...
   {c?<div className="hm-card hm-refs">{c.storage&&<p><b>Storage · </b>{c.storage}</p>}<p>{c.source}</p><a href={google(`${c.source} ${m.name} recipe`)} target="_blank" rel="noreferrer">Find written source ↗</a>{c.youtube&&<a href={youtube(c.youtube)} target="_blank" rel="noreferrer">Watch: {c.youtube} ↗</a>}</div>
   :f&&f.evidence.length>0&&<div className="hm-card hm-refs">{f.evidence.map(e=><a key={e.url} href={e.url} target="_blank" rel="noreferrer">{e.label} ↗</a>)}</div>}
