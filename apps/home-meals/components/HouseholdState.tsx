...
type HouseholdState={
 week:string[]; componentStock:Record<string,number>; ingredientStock:Record<string,number>; groceryChecked:Record<string,boolean>; ratings:Record<string,Rating>; recipeNotes:Record<string,RecipeNote[]>; history:CookEvent[]; prepBatches:PrepBatch[]; kitchenReady:boolean;
...
export function HouseholdStateProvider({children}:{children:React.ReactNode}){
 ...
 useEffect(()=>{try{
  ...
  if(raw){const s=JSON.parse(raw);
   ...
   // Only V7 stock is trusted. Earlier releases contained demo inventory or different stock units.
   if(exact){if(s.componentStock)setComponentStock({...initialComponentStock,...s.componentStock});if(s.ingredientStock)setIngredientStock({...initialIngredientStock,...s.ingredientStock});setKitchenReady(!!s.kitchenReady);if(s.prepBatches)setPrepBatches(s.prepBatches)}
 ...
 useEffect(()=>{if(!hydrated)return;localStorage.setItem(KEY,JSON.stringify({version:7,week,componentStock,ingredientStock,groceryChecked,ratings,recipeNotes,history,prepBatches,kitchenReady}))},[hydrated,week,componentStock,ingredientStock,groceryChecked,ratings,recipeNotes,history,prepBatches,kitchenReady]);
 ...
 const resetDemo=()=>{setWeek(defaultWeek);setComponentStock(initialComponentStock);setIngredientStock(initialIngredientStock);setGroceryChecked({});setRatings({});setRecipeNotes({});setHistory([]);setPrepBatches([]);setKitchenReady(false)};
 const value={week,componentStock,ingredientStock,groceryChecked,ratings,recipeNotes,history,prepBatches,kitchenReady,prepNeeds,shoppingNeeds,setDay,setComponent,setIngredient,toggleGrocery,makeBatch,cookMeal,rateMeal,noteMeal,confirmKitchen,resetDemo};
