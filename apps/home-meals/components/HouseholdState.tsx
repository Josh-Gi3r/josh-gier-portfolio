...
const legacyToCanonical:Record<string,string>={
 eggs:"egg-large","basmati-rice":"basmati-rice-dry","jasmine-rice":"jasmine-rice-dry",rice:"jasmine-rice-dry",beef:"beef-sirloin","ground-meat":"ground-chicken",mushrooms:"mushroom",onion:"onion-yellow",potatoes:"potato",chickpeas:"chickpeas-drained",tortillas:"tortilla",butter:"butter-unsalted","spring-onion":"spring-onion"
};
...
function Bridge({children}:{children:React.ReactNode}){
 const v=useHouseholdV12();
 const[mealPhotos,setMealPhotos]=useState<MealPhoto[]>([]);const[storageIssue,setStorageIssue]=useState(false);
 ...
 const setComponent=(id:string,qty:number)=>{const c=getCanonicalPrepV2(id);if(!c)return;v.reconcileComponentTotal(id,quantity(Math.max(0,qty),c.workingUnit.unit))};
 const setIngredient=(id:string,qty:number)=>{const item=getIngredient(id);if(item?.tracking==="state"){v.setQualitativeIngredientLevel(id,qty);return}const key=canonicalIdForUi(id,item?.unit);const def=getCanonicalIngredientV2(key);if(!def){setStorageIssue(true);return}v.setIngredientObserved(key,quantity(Math.max(0,qty),def.canonicalUnit))};
