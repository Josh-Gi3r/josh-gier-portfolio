...
import {
 componentConsumption, defaultWeek, getComponent, getIngredient, initialComponentStock, initialIngredientStock,
...
const KEY="home-meals-household-v5";
const LEGACY_KEYS=["home-meals-household-v4","home-meals-linked-v3"];
...
export function HouseholdStateProvider({children}:{children:React.ReactNode}){
 ...
 useEffect(()=>{try{
  ...
  const legacy=!exact?LEGACY_KEYS.map(k=>localStorage.getItem(k)).find(Boolean):null;
  const raw=exact??legacy;
  if(raw){const s=JSON.parse(raw);
   ...
   // Only V5 stock is trusted. Earlier builds shipped demo inventory that looked like real household stock.
 ...
 useEffect(()=>{if(!hydrated)return;localStorage.setItem(KEY,JSON.stringify({version:5,week,componentStock,ingredientStock,groceryChecked,ratings,recipeNotes,history,kitchenReady}))},[hydrated,week,componentStock,ingredientStock,groceryChecked,ratings,recipeNotes,history,kitchenReady]);
 ...
 const setComponent=(id:string,qty:number)=>{setComponentStock(prev=>({...prev,[id]:Math.max(0,qty)}));setKitchenReady(true)};
 ...
 const makeBatch=(id:string)=>{const item=getComponent(id);const yieldQty=item?.batchYield??1;setComponentStock(prev=>({...prev,[id]:(prev[id]??0)+yieldQty}));setKitchenReady(true)};
