...
export function HouseholdStateProvider({children}:{children:React.ReactNode}){
 const [week,setWeek]=useState<string[]>(defaultWeek);const [monthlyPool,setMonthlyPool]=useState<string[]>(defaultMonthPool);const [componentStock,setComponentStock]=useState<Record<string,number>>(initialComponentStock);const [ingredientStock,setIngredientStock]=useState<Record<string,number>>(initialIngredientStock);const [groceryChecked,setGroceryChecked]=useState<Record<string,boolean>>({});const [ratings,setRatings]=useState<Record<string,Rating>>({});const [recipeNotes,setRecipeNotes]=useState<Record<string,RecipeNote[]>>({});const [history,setHistory]=useState<CookEvent[]>([]);const [prepBatches,setPrepBatches]=useState<PrepBatch[]>([]);const [kitchenReady,setKitchenReady]=useState(false);const [useSoon,setUseSoon]=useState<Record<string,boolean>>({});const [favourites,setFavourites]=useState<Record<string,boolean>>({});const [hydrated,setHydrated]=useState(false);
 useEffect(()=>{try{const exact=localStorage.getItem(KEY);const legacy=!exact?LEGACY_KEYS.map(k=>localStorage.getItem(k)).find(Boolean):null;const raw=exact??legacy;if(raw){const s=JSON.parse(raw);if(Array.isArray(s.week)){const migrated=s.week.filter((x:string)=>validRecipeIds.has(x));setWeek(migrated.length===7?migrated:defaultWeek)}if(Array.isArray(s.monthlyPool)){const pool=s.monthlyPool.filter((x:string)=>validRecipeIds.has(x));if(pool.length)setMonthlyPool(pool)}if(exact){if(s.componentStock)setComponentStock({...initialComponentStock,...s.componentStock});if(s.ingredientStock)setIngredientStock({...initialIngredientStock,...s.ingredientStock});setKitchenReady(!!s.kitchenReady);if(s.prepBatches)setPrepBatches((s.prepBatches as any[]).map(b=>({...b,remainingMl:b.remainingMl??b.outputMl})));if(s.useSoon)setUseSoon(s.useSoon);if(s.favourites)setFavourites(s.favourites)}if(s.groceryChecked)setGroceryChecked(s.groceryChecked);if(s.ratings){setRatings(s.ratings);const notes:Record<string,RecipeNote[]>={};for(const [id,r] of Object.entries(s.ratings as Record<string,Rating>)){if(r.note)notes[id]=[{author:"home",text:r.note,at:new Date().toISOString()}]}setRecipeNotes(s.recipeNotes??notes)}if(s.recipeNotes)setRecipeNotes(s.recipeNotes);if(s.history)setHistory((s.history as CookEvent[]).filter(x=>validRecipeIds.has(x.mealId)))}}catch{}finally{setHydrated(true)}},[]);
