...
export function HouseholdStatusCard(){
 ...
 const rows=[
  ...
  {label:"Ask Home",on:!!status?.askHome,note:status?.askHome?(status.model??"GPT-5.6 Sol"):"Local household logic"},
  {label:"Camera intelligence",on:!!status?.vision,note:status?.vision?(status.visionModel??"GPT-5.6 Sol"):"Manual confirmation"},
  {label:"GPT-Live voice",on:!!status?.realtimeVoice,note:status?.realtimeVoice?(status.liveModel??"gpt-live-1"):"Browser speech fallback"},
 ];
