...
export type PrepJobV2=Readonly<{componentId:string;shortfall:Quantity;madeFrom:readonly string[];storageMode:string;phase:"long"|"cooked-paste"|"raw-paste"|"quick-fridge"|"pantry";outputMustBeMeasured:true;parentQuantityStatus:"explicit_formulation_required"|"not_applicable"}>;
...
export function prepJobsForWeekV2(recipeIds:readonly string[],stock:ComponentStockV2):PrepJobV2[]{
 const needs=prepNeedsForRecipesV2(recipeIds,stock),directIds=new Set(needs.map(x=>x.componentId)),expanded=new Set(directIds);const visit=(id:string)=>{const c=getCanonicalPrepV2(id);if(!c)return;for(const parent of c.madeFrom)if(!expanded.has(parent)){expanded.add(parent);visit(parent)}};for(const id of [...directIds])visit(id);const needById=new Map(needs.map(x=>[x.componentId,x.shortfall]));
 const jobs:PrepJobV2[]=[...expanded].map((id):PrepJobV2=>{const c=getCanonicalPrepV2(id);if(!c)throw new Error(`Unknown prep component ${id}`);const shortfall=needById.get(id)??{qty:0,unit:c.workingUnit.unit};const parentQuantityStatus:PrepJobV2["parentQuantityStatus"]=c.madeFrom.length?"explicit_formulation_required":"not_applicable";return{componentId:id,shortfall,madeFrom:c.madeFrom,storageMode:c.storageMode,phase:phaseFor(id),outputMustBeMeasured:true,parentQuantityStatus}});
 const order:Record<PrepJobV2["phase"],number>={long:0,"cooked-paste":1,"raw-paste":2,"quick-fridge":3,pantry:4};return jobs.sort((a,b)=>{const pa=order[a.phase],pb=order[b.phase];if(pa!==pb)return pa-pb;const ac=getCanonicalPrepV2(a.componentId),bc=getCanonicalPrepV2(b.componentId);if(ac&&bc&&bc.madeFrom.includes(ac.id))return-1;if(ac&&bc&&ac.madeFrom.includes(bc.id))return 1;return a.componentId.localeCompare(b.componentId)});
}
