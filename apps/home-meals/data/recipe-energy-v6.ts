import {getOriginalRuntimeDinnerV8} from "./runtime-original-v8";
...
function confidenceRank(c:EnergyConfidenceV6|"B"|"C"|"D"){const s=String(c);return s.startsWith("D")?3:s.startsWith("C")?2:1}
function adjustIngredientKcal(recipeId:string,row:NormalIngredient,kcal:number){
 ...
 if(/oil/.test(s)&&/not all|fry|frying|shallow/.test(s)){const factor=deepFryIds.has(recipeId)?0.15:shallowFryIds.has(recipeId)?0.25:0.2;return kcal*factor}
...
function explicitEnergyFallbackV6(row:NormalIngredient):FallbackReference|null{
 ...
 switch(id){
  ...
  case "malaysian-meat-curry-powder":case "meat-curry-powder":return u==="g"?F("g",3.0,"Dry meat-curry-powder blend proxy; contribution is small."):null;
...
function ingredientKcalV6(row:NormalIngredient){if(row.optional)return ingredientReferenceKcalV6(row);const exact=explicitEnergyFallbackV6(row);if(exact&&exact.unit===row.unit)return{kcal:Math.round(row.qty*exact.kcalPerUnit*10)/10,reference:exact,excludedOptional:false};return ingredientReferenceKcalV6(row)}
export function estimateRecipeEnergyV6(args:{recipeId:string;servings:number;ingredients:readonly NormalIngredient[];prep:readonly NormalPrep[]}):RecipeEnergyReferenceV6{
 let ingredientKcal=0,prepKcal=0,rawIngredientKcal=0,optionalIngredientsExcluded=0,worst=1;const unresolvedIngredientIds:string[]=[],unresolvedPrepIds:string[]=[];
 for(const row of args.ingredients){const result=ingredientKcalV6(row);if(result.excludedOptional){optionalIngredientsExcluded++;continue}if(result.kcal==null||!result.reference){unresolvedIngredientIds.push(row.ingredientId??row.id??row.name);continue}rawIngredientKcal+=result.kcal;ingredientKcal+=adjustIngredientKcal(args.recipeId,row,result.kcal);worst=Math.max(worst,confidenceRank(result.reference.confidence))}
...
function liveReference(recipeId:string){const f=getOriginalRuntimeDinnerV8(recipeId);if(!f)throw new Error(`Missing four-serving runtime formulation for ${recipeId}`);return estimateRecipeEnergyV6({recipeId,servings:f.targetServings,ingredients:f.ingredients.map(x=>({ingredientId:x.ingredientId,name:x.name,qty:x.qty,unit:x.unit,optional:x.optional,basis:x.basis,note:x.note})),prep:f.prep.map(x=>({componentId:x.componentId,qty:x.quantity.qty,unit:x.quantity.unit}))})}
...
export const phase2RecipeEnergyReferencesV6:Readonly<Record<string,RecipeEnergyReferenceV6>>=Object.fromEntries(phase2OperationalResearchRecipesV8.map(r=>[r.id,estimateRecipeEnergyV6({recipeId:r.id,servings:r.targetServings,ingredients:r.ingredients.map(x=>({id:x.id,name:x.name,qty:x.qty,unit:x.unit,optional:x.optional,basis:x.basis,note:x.note})),prep:r.prep.map(x=>({componentId:x.componentId,qty:x.qty,unit:x.unit}))})] as const));
export const allRecipeEnergyReferencesV6:Readonly<Record<string,RecipeEnergyReferenceV6>>={...liveRecipeEnergyReferencesV6,...phase2RecipeEnergyReferencesV6};export function recipeEnergyReferenceV6(recipeId:string){return allRecipeEnergyReferencesV6[recipeId]}
export function validateRecipeEnergyReferencesV6(){const errors:string[]=[];const ids=Object.keys(allRecipeEnergyReferencesV6);if(Object.keys(liveRecipeEnergyReferencesV6).length!==36)errors.push(`Expected 36 live energy references, found ${Object.keys(liveRecipeEnergyReferencesV6).length}`);if(Object.keys(phase2RecipeEnergyReferencesV6).length!==100)errors.push(`Expected 100 Phase 2 energy references, found ${Object.keys(phase2RecipeEnergyReferencesV6).length}`);if(ids.length!==136)errors.push(`Expected 136 total energy references, found ${ids.length}`);for(const r of Object.values(allRecipeEnergyReferencesV6)){if(r.unresolvedIngredientIds.length)errors.push(`${r.recipeId} unresolved ingredients: ${r.unresolvedIngredientIds.join(",")}`);if(r.unresolvedPrepIds.length)errors.push(`${r.recipeId} unresolved prep: ${r.unresolvedPrepIds.join(",")}`);if(!(r.kcalPerPerson>0))errors.push(`${r.recipeId} invalid kcal/person`);if(r.servings!==4)errors.push(`${r.recipeId} is not calculated over four servings`)}return{valid:errors.length===0,errors}}
