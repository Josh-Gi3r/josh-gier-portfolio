...
/** neededPortions now means practical V6 storage packets/doses, not the legacy tiny workingUnit. */
...
export function stockPortions(id:string,stock:Record<string,number>){const p=getPrepPortionPolicyV6(id),c=getCanonicalPrepV2(id);if(!p||!c)return 0;return Math.floor(Math.max(0,stock[id]??0)/Math.max(1,p.packet.qty))}
export function stockPacketCountExact(id:string,stock:Record<string,number>){const p=getPrepPortionPolicyV6(id);if(!p)return 0;return packetCountV6(id,quantity(Math.max(0,stock[id]??0),p.packet.unit))}
