...
/** Only use aliases where one legacy label truly represents different measured forms. Never convert g↔ml. */
export function canonicalIngredientKeyV2(id:string,unit:QuantityUnit){
  if(id==="tamarind"&&unit==="g")return "tamarind-paste";
  if(id==="tamarind"&&unit==="ml")return "tamarind-water";
  return id;
}
...
const allergenById:Readonly<Record<string,readonly MajorAllergenV2[]>>={
  ...
  "sesame-oil":["sesame"],"sesame-seed":["sesame"],"butter-unsalted":["milk"],"cream":["milk"],"yoghurt":["milk"],"parmesan":["milk"],
  "egg-large":["egg"],"peanuts":["peanut"],"cashew":["tree-nut"],"candlenut":["tree-nut"],"pine-nut-or-cashew":["tree-nut"],"flour-plain":["wheat"],"dijon":["mustard"],
};
...
export const canonicalIngredientCatalogV2:readonly CanonicalIngredientCatalogV2[]=[...rows.entries()].map(([id,row])=>{const units=[...row.units];const canonicalUnit=units[0];return{id,name:row.name,canonicalUnit,allergens:allergenById[id]??[],nutritionBindingClass:manufacturerRequired.has(id)?"manufacturer_required":manufacturerPreferred.has(id)?"manufacturer_preferred":id==="water"?"non_nutritive_or_process":"fdc_generic",sourcing:sourcingById[id]??(["normal-supermarket"] as const),specialistRisk:specialistRiskIds.has(id)}}).sort((a,b)=>a.id.localeCompare(b.id));
