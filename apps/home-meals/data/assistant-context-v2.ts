...
export function buildAssistantContextV2(state:HouseholdStateV12){
  ...
  const recipes=canonicalRecipesV2.map(recipe=>{
    ...
    return {id:recipe.id,title:recipe.title,cuisine:recipe.cuisine,reportedMinutes:recipe.reportedTotalMinutes,kcalReferencePerPerson:energy?.kcalPerPerson??null,kcalUncertaintyPct:energy?.uncertaintyPct??null,mealWeight:energy?.mealWeight??null,prepReady:prep.ready,ingredientReady:ingredients.ready,activePrepFit:outside.length===0,missingActivePrep:outside,history:hx,missingPrep:prep.missing.map(x=>({id:x.componentId,shortfall:formatQuantity(x.shortfall)})),missingIngredients:ingredients.missing.map(x=>({id:x.ingredientId,shortfall:formatQuantity(x.shortfall)})),allergens:allergens.allergens,variants:recipeVariantsForV2(recipe.id).map(x=>({id:x.id,name:x.name}))};
  });
  ...
  return {
    ...
    truthRules:{inventory:"kitchenReady=false means unknown, not empty. A confirmed empty kitchen has kitchenReady=true with zero stock.",repertoire:"The 41 prep components are a capability library, not a checklist. Prefer the household's activePrepIds and explain when one extra prep item would unlock meaningful variety.",history:"Use deterministic cook history to avoid boring repetition and answer what was eaten recently.",nutrition:"Recipe kcalReferencePerPerson is a rounded planning estimate with the supplied uncertainty, not calibrated household nutrition. Quote it as an estimate when useful; never invent missing kcal or macros. Exact component nutrition still requires measured finished output/product evidence.",safety:"Use structured safety targets; visual appearance never proves safe internal temperature.",prepStock:"Home does not need the total cooked batch weight. Stock may come from household-confirmed standardized working portions, where each portion maps to the component's canonical working unit. Never invent how many portions a batch made.",qualitative:"Out/Low/Some/Plenty is presence-level truth only; never translate it into grams or millilitres.",units:"Never convert g and ml without an explicit measured conversion, which Home Meals currently does not define."},
  };
}
