...
export function buildAssistantContextV2(state:HouseholdStateV12){
  ...
  return {
    ...
    truthRules:{inventory:"kitchenReady=false means unknown, not empty. A confirmed empty kitchen has kitchenReady=true with zero stock.",repertoire:"The 41 prep components are a capability library, not a checklist. Prefer the household's activePrepIds and explain when one extra prep item would unlock meaningful variety.",history:"Use deterministic cook history to avoid boring repetition and answer what was eaten recently.",nutrition:"Only state a number supplied by the deterministic nutrition engine.",safety:"Use structured safety targets; visual appearance never proves safe internal temperature.",yield:"Only measured household batch output is stock truth.",qualitative:"Out/Low/Some/Plenty is presence-level truth only; never translate it into grams or millilitres.",units:"Never convert g and ml without an explicit measured conversion, which Home Meals currently does not define."},
  };
}
