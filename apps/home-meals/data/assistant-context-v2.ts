...
export function buildAssistantContextV2(state:HouseholdStateV12){
  ...
  const recipes=canonicalRecipesV2.map(recipe=>{
    ...
    return {id:recipe.id,title:recipe.title,cuisine:recipe.cuisine,servings:recipe.servings,reportedMinutes:recipe.reportedTotalMinutes,kcalReferencePerPerson:energy?.kcalPerPerson??null,kcalUncertaintyPct:energy?.uncertaintyPct??null,mealWeight:energy?.mealWeight??null,prepReady:prep.ready,ingredientReady:ingredients.ready,activePrepFit:outside.length===0,missingActivePrep:outside,history:hx,missingPrep:prep.missing.map(x=>({id:x.componentId,shortfall:formatQuantity(x.shortfall)})),missingIngredients:ingredients.missing.map(x=>({id:x.ingredientId,shortfall:formatQuantity(x.shortfall)})),allergens:allergens.allergens,variants:recipeVariantsForV2(recipe.id).map(x=>({id:x.id,name:x.name}))};
  });
