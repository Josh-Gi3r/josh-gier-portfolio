...
export type HouseholdStateV12 = {
  ...
  ratings:Record<string,RatingV12>;
  recipeNotes:Record<string,RecipeNoteV12[]>;
  recipeVersions:Record<string,RecipeVersionV12[]>;
  ...
  legacyArchive?:{
    ...
    groceryCheckedV11?:Record<string,boolean>;
  };
...
type LegacyHouseholdV11 = Partial<{
  ...
  ratings:Record<string,RatingV12>;recipeNotes:Record<string,RecipeNoteV12[]>;recipeVersions:Record<string,RecipeVersionV12[]>;history:{mealId:string;at:string}[];
...
function cleanNotes(value:unknown){const out:Record<string,RecipeNoteV12[]>={};for(const [id,rows] of Object.entries(safeRecord<RecipeNoteV12[]>(value))){if(!Array.isArray(rows))continue;out[id]=rows.filter(x=>x&&typeof x.text==="string").map(x=>({author:x.author==="josh"||x.author==="g"?x.author:"home",text:x.text.trim().slice(0,500),at:x.at||new Date().toISOString()})).slice(0,20)}return out}
function cleanVersions(value:unknown){const out:Record<string,RecipeVersionV12[]>={};for(const [id,rows] of Object.entries(safeRecord<RecipeVersionV12[]>(value))){if(!Array.isArray(rows))continue;out[id]=rows.filter(x=>x&&Number.isFinite(Number(x.number))&&typeof x.summary==="string").map(x=>({number:Math.max(1,Number(x.number)),summary:x.summary.trim().slice(0,500),author:x.author==="josh"||x.author==="g"?x.author:"home",at:x.at||new Date().toISOString()})).slice(0,20)}return out}
...
export function migrateHouseholdV11ToV12(legacy:LegacyHouseholdV11,defaults:{week:string[];monthlyPool:string[]}):HouseholdStateV12 {
  const componentStockV11=safeRecord<number>(legacy.componentStock);const ingredientStockV11=safeRecord<number>(legacy.ingredientStock);const groceryCheckedV11=safeRecord<boolean>(legacy.groceryChecked);
  ...
  return {
    ...
    componentBatches:[],manualComponentStock:emptyManualComponentStockV12(),ingredientStock:emptyIngredientStockV12(),groceryChecked:{},ratings:cleanRatings(legacy.ratings),recipeNotes:cleanNotes(legacy.recipeNotes),recipeVersions:cleanVersions(legacy.recipeVersions),
    history:Array.isArray(legacy.history)?legacy.history.filter(x=>x&&typeof x.mealId==="string").map(x=>({mealId:x.mealId,at:x.at||new Date().toISOString()})):[],useSoon,useSoonAt,favourites:safeRecord<boolean>(legacy.favourites),kitchenReady:!!legacy.kitchenReady,
    legacyArchive:{componentStockV11,ingredientStockV11,prepBatchesV11:prepBatches,groceryCheckedV11},migrationWarnings:warnings,
  };
}
...
export function cookRecipeV12(state:HouseholdStateV12,recipeId:string,variantId?:string,at=new Date().toISOString()):HouseholdStateV12{const ingredientAvailability=ingredientAvailabilityForRecipeV2(recipeId,state.ingredientStock,variantId);if(!ingredientAvailability.ready)throw new Error(`Insufficient ingredient stock for ${recipeId}`);const afterPrep=consumeRecipePrepV12(state,recipeId);const ingredientStock=consumeRecipeIngredientsV2(recipeId,afterPrep.ingredientStock,variantId);return {...afterPrep,ingredientStock,history:[{mealId:recipeId,variantId,at},...afterPrep.history].slice(0,100),kitchenReady:true}}
