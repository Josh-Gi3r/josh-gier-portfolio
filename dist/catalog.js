...
export const products=[...originalProducts.map(p=>({...p,collection:'Products',...productDetails[p.id]})),...openSource].map(p=>({...p,...productCopy[p.id]}));
...
export const campaigns=originalCampaigns.map(p=>({...p,...changes[p.id],...campaignCopy[p.id]}));
